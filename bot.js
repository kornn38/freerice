function FRBot() {
  this.runDuration = 1; // runtime hours set
  this.maxRice = -1;
  this.riceDonated = 0;
  this.started = 0;
  this.timeoutId = 0;
  this.iterations = 0;
  this.paused = false;
  FRthis = this;

  var lastQtn, lastType;

  this.run = function () {
    this.started = Date.now();
    this.iterations = this.riceDonated = 0;
    this.runLoop();
    if (this.runDuration > 0)
      setTimeout(function () {
        FRthis.stop();
      }, 1000 * 60 * 60 * this.runDuration);
  };

  this.runLoop = function () {
    /* Answer a question and schedule answering the next one in some random amount of time */
    this.paused = false;
    lastType = null;
    this.getRice();
    this.timeoutId = setTimeout(function () {
      FRthis.runLoop();
    }, Math.random() * 2600 +
      4200 +
      (Math.random() < 0.03 ? Math.random() * 30000 : 0));
    this.iterations++;
  };

  this.pause = function () {
    this.paused = true;
    clearTimeout(this.timeoutId);
  };
  this.resume = this.runLoop;

  this.getRice = function () {
    var type = document.getElementsByClassName("rice-counter__category-text")[0]
        .innerText,
      Answer;
    var question = document
      .getElementsByClassName("card-title")[0]
      .innerHTML.split(" =")[0];

    this.riceDonated = parseInt(
      document.getElementsByClassName("rice-counter__value")[0].innerText
    );

    /* Check for return conditions */
    if (this.maxRice > 0 && this.riceDonated >= this.maxRice)
      return this.stop();

    /* Select an answer to the question presented */
    if (type.indexOf("Chemical Sym") != -1) {
      Answer = ElementLookup[question];
    } else if (
      type.indexOf("Vocab") != -1 ||
      type.indexOf("Spanish") != -1 ||
      type.indexOf("Italian") != -1 ||
      type.indexOf("French") != -1 ||
      type.indexOf("German") != -1
    ) {
      lastType = "Vocab";
      lastQtn = question.split(" means")[0].toLowerCase();
      Answer = VocabAnswers[lastQtn];
    } else if (type.indexOf("World Capitals") != -1) {
      lastType = "Capitals";
      lastQtn = question.split(" is capital of")[0].toLowerCase();
      Answer = CapitalAnswers[lastQtn];
    } else if (
      type.indexOf("Identify Countries") != -1 ||
      type.indexOf("Flags") != -1 ||
      type.indexOf("World Landmarks") != -1 ||
      type.indexOf("Famous Paintings") != -1
    ) {
      lastType = "Img";
      lastQtn = document
        .getElementsByClassName("question-image")[0]
        .firstChild.src.split("/")
        .pop()
        .split(".")[0]
        .replace(/[^a-z0-9]/gi, "_");
      Answer = ImgAnswerKey[lastQtn];
    }

    if (Answer == null) Answer = 1 + Math.round(Math.random() * 3);

    //console.log("Answer="+Answer);

    /* Check each possible answer and choose the matching one */
    /* Note: randomly getting a small percentage incorrect to mimic human error */
    var clicked = false;
    Array.from(document.getElementsByClassName("fade-appear-done")).forEach(
      function (curr, index) {
        //console.log("Option "+(index+1)+"="+curr.innerText);
        if (
          !clicked &&
          (index == 3 ||
            Math.random() < 0.005 / 4 ||
            (Answer != null &&
              (Answer == index + 1 || Answer == curr.innerText)) ||
            (type == "Multiplication Table" &&
              curr.innerText == eval(status.replace("x", "*"))))
        ) {
          curr.firstChild.click();
          clicked = true;
        }
      }
    );
    setTimeout(function () {
      FRthis.storeAnswer();
    }, 2000);
  };

  this.storeAnswer = function () {
    /* Have to check answer some time after the click by looking for a 'correct' class added to one of the answers */
    if (lastType != undefined) {
      var lastAnswer;
      Array.from(document.getElementsByClassName("fade-appear-done")).forEach(
        function (curr, index) {
          if (Array.from(curr.firstChild.classList).indexOf("correct") != -1)
            lastAnswer = curr.firstChild.innerText;
        }
      );
      //console.log(lastType+"["+lastQtn+"]="+lastAnswer);

      if (lastType === "Vocab") VocabAnswers[lastQtn] = lastAnswer;
      else if (lastType === "Capitals") CapitalAnswers[lastQtn] = lastAnswer;
      else if (lastType === "Img") ImgAnswerKey[lastQtn] = lastAnswer;
    }
  };

  this.stop = function () {
    clearTimeout(this.timeoutId);
    this.started = 0;
  };

  this.status = function () {
    if (this.started != 0) {
      var info = this.paused ? "(PAUSED)\n" : "";
      var elapsedMinutes = (Date.now() - this.started) / (1000 * 60);
      info = info.concat("Running for ");
      if (elapsedMinutes <= 59)
        info = info.concat(Math.floor(elapsedMinutes) + " minutes\n");
      else info = info.concat((elapsedMinutes / 60).toFixed(2) + " hours\n");
      info = info.concat("Rice donated: " + this.riceDonated + "\n");
      info = info.concat("Questions seen: " + this.iterations + "\n");
      if (this.maxRice != -1)
        info = info.concat("Max rice to collect: " + this.maxRice + "\n");
      if (this.runDuration > 0) {
        var timeLeft = this.runDuration * 60 - elapsedMinutes;
        if (timeLeft >= 60)
          info = info.concat(
            "Time left: " + (timeLeft / 60).toFixed(2) + " hours\n"
          );
        else
          info = info.concat(
            "Time left: " + timeLeft.toFixed(2) + " minutes\n"
          );
      }
      console.log(info);
    } else console.log("Stopped");
  };
}

/* Chem. symbol key */
var ElementLookup = {
  Actinium: "Ac",
  Aluminium: "Al",
  Aluminum: "Al",
  Americium: "Am",
  Antimony: "Sb",
  Argon: "Ar",
  Arsenic: "As",
  Astatine: "At",
  Barium: "Ba",
  Berkelium: "Bk",
  Beryllium: "Be",
  Bismuth: "Bi",
  Bohrium: "Bh",
  Boron: "B",
  Bromine: "Br",
  Cadmium: "Cd",
  Calcium: "Ca",
  Californium: "Cf",
  Carbon: "C",
  Cerium: "Ce",
  Cesium: "Cs",
  Chlorine: "Cl",
  Chromium: "Cr",
  Cobalt: "Co",
  Copper: "Cu",
  Curium: "Cm",
  Darmstadtium: "Ds",
  Dubnium: "Db",
  Dysprosium: "Dy",
  Einsteinium: "Es",
  Erbium: "Er",
  Europium: "Eu",
  Fermium: "Fm",
  Fluorine: "F",
  Francium: "Fr",
  Gadolinium: "Gd",
  Gallium: "Ga",
  Germanium: "Ge",
  Gold: "Au",
  Hafnium: "Hf",
  Hassium: "Hs",
  Helium: "He",
  Holmium: "Ho",
  Hydrogen: "H",
  Indium: "In",
  Iodine: "I",
  Iridium: "Ir",
  Iron: "Fe",
  Krypton: "Kr",
  Lanthanum: "La",
  Lawrencium: "Lr",
  Lead: "Pb",
  Lithium: "Li",
  Lutetium: "Lu",
  Magnesium: "Mg",
  Manganese: "Mn",
  Meitnerium: "Mt",
  Mendelevium: "Md",
  Mercury: "Hg",
  Molybdenum: "Mo",
  Neodymium: "Nd",
  Neon: "Ne",
  Neptunium: "Np",
  Nickel: "Ni",
  Niobium: "Nb",
  Nitrogen: "N",
  Nobelium: "No",
  Osmium: "Os",
  Oxygen: "O",
  Palladium: "Pd",
  Phosphorus: "P",
  Platinum: "Pt",
  Plutonium: "Pu",
  Polonium: "Po",
  Potassium: "K",
  Praseodymium: "Pr",
  Promethium: "Pm",
  Protactinium: "Pa",
  Radium: "Ra",
  Radon: "Rn",
  Rhenium: "Re",
  Rhodium: "Rh",
  Rubidium: "Rb",
  Ruthenium: "Ru",
  Rutherfordium: "Rf",
  Samarium: "Sm",
  Scandium: "Sc",
  Seaborgium: "Sg",
  Selenium: "Se",
  Silicon: "Si",
  Silver: "Ag",
  Sodium: "Na",
  Strontium: "Sr",
  Sulfur: "S",
  Tantalum: "Ta",
  Technetium: "Tc",
  Tellurium: "Te",
  Terbium: "Tb",
  Thallium: "Tl",
  Thorium: "Th",
  Thulium: "Tm",
  Tin: "Sn",
  Titanium: "Ti",
  Tungsten: "W",
  Ununbium: "Uub",
  Ununhexium: "Uuh",
  Ununium: "Uuu",
  Ununoctium: "Uuo",
  Ununpentium: "Uup",
  Ununquadium: "Uuq",
  Ununseptium: "Uus",
  Ununtrium: "Uut",
  Uranium: "U",
  Vanadium: "V",
  Xenon: "Xe",
  Ytterbium: "Yb",
  Yttrium: "Y",
  Zinc: "Zn",
  Zirconium: "Zr",
};

/* Image question-type answer key */
var ImgAnswerKey = {
  ac_lgflag: "Antigua and Barbuda",
  af_lgflag_0: "Afghanistan",
  ag_lgflag_0: "Algeria",
  aj_lgflag: "Azerbaijan",
  al_lgflag_0: "Albania",
  am_lgflag: "Armenia",
  ao_lgflag: "Angola",
  as_lgflag: "Australia",
  au_lgflag: "Austria",
  ba_lgflag: "Bahrain",
  bb_lgflag: "Barbados",
  be_lgflag: "Belgium",
  bf_lgflag: "Bahamas",
  bg_lgflag: "Bangladesh",
  bh_lgflag: "Belize",
  bk_lgflag: "Bosnia and Herzegovina",
  bl_lgflag: "Bolivia",
  bo_lgflag: "Belarus",
  br_lgflag: "Brazil",
  bt_lgflag: "Bhutan",
  by_lgflag: "Burundi",
  cb_lgflag: "Cambodia",
  cd_lgflag: "Chad",
  ce_lgflag: "Sri Lanka",
  cg_lgflag: "Democratic Republic of the Congo",
  ci_lgflag: "Chile",
  cn_lgflag: "Comoros",
  co_lgflag: "Colombia",
  ct_lgflag: "Central African Republic",
  cu_lgflag: "Cuba",
  cv_lgflag: "Cape Verde",
  cy_lgflag: "Cyprus",
  da_lgflag: "Denmark",
  dj_lgflag: "Djibouti",
  dr_lgflag: "Dominican Republic",
  ec_lgflag: "Ecuador",
  eg_lgflag: "Egypt",
  ei_lgflag: "Ireland",
  er_lgflag: "Eritrea",
  es_lgflag: "El Salvador",
  et_lgflag: "Ethiopia",
  flag_of_malawi: "Malawi",
  gg_lgflag: "Georgia",
  gh_lgflag: "Ghana",
  gj_lgflag: "Grenada",
  gm_lgflag: "Germany",
  gt_lgflag: "Guatemala",
  gv_lgflag: "Guinea",
  gy_lgflag: "Guyana",
  ha_lgflag: "Haiti",
  hr_lgflag: "Croatia",
  hu_lgflag: "Hungary",
  id_lgflag: "Indonesia",
  is_lgflag: "Israel",
  ja_lgflag: "Japan",
  jm_lgflag: "Jamaica",
  jo_lgflag: "Jordan",
  ke_lgflag: "Kenya",
  kr_lgflag: "Kiribati",
  ks_lgflag: "Republic of Korea",
  la_lgflag: "Lao People's Democratic Republic",
  le_lgflag: "Lebanon",
  lg_lgflag: "Latvia",
  lo_lgflag: "Slovakia",
  lu_lgflag: "Luxembourg",
  ly_lgflag: "Libya",
  ma_lgflag: "Madagascar",
  md_lgflag: "Republic of Moldova",
  mg_lgflag: "Mongolia",
  mn_lgflag: "Monaco",
  mo_lgflag: "Morocco",
  mp_lgflag: "Mauritius",
  mr_lgflag: "Mauritania",
  mt_lgflag: "Malta",
  mu_lgflag: "Oman",
  mx_lgflag: "Mexico",
  my_lgflag: "Malaysia",
  ng_lgflag: "Niger",
  nh_lgflag: "Vanuatu",
  ni_lgflag: "Nigeria",
  nr_lgflag: "Nauru",
  ns_lgflag: "Suriname",
  nz_lgflag: "New Zealand",
  pa_lgflag: "Paraguay",
  pm_lgflag: "Panama",
  po_lgflag: "Portugal",
  pp_lgflag: "Papua New Guinea",
  pu_lgflag: "Guinea-Bissau",
  rm_lgflag: "Marshall Islands",
  ro_lgflag: "Romania",
  rs_lgflag: "Russian Federation",
  rw_lgflag: "Rwanda",
  sa_lgflag: "Saudi Arabia",
  sc_lgflag: "Saint Kitts and Nevis",
  se_lgflag: "Seychelles",
  sg_lgflag: "Senegal",
  si_lgflag: "Slovenia",
  sl_lgflag: "Sierra Leone",
  sn_lgflag: "Singapore",
  so_lgflag: "Somalia",
  sp_lgflag: "Spain",
  st_lgflag: "Saint Lucia",
  sy_lgflag: "Syrian Arab Republic",
  th_lgflag: "Thailand",
  ts_lgflag: "Tunisia",
  tt_lgflag: "Timor-Leste",
  tz_lgflag: "United Republic of Tanzania",
  up_lgflag: "Ukraine",
  uv_lgflag: "Burkina Faso",
  uy_lgflag: "Uruguay",
  uz_lgflag: "Uzbekistan",
  vc_lgflag: "Saint Vincent and the Grenadines",
  ve_lgflag: "Venezuela (Bolivarian Republic of)",
  vm_lgflag: "Viet Nam",
  wa_1: "Egypt",
  wa_10: "Tunisia",
  wa_100: "Moscow",
  wa_101: "Moscow",
  wa_103: "Romania",
  wa_104: "Portugal",
  wa_105: "Portugal",
  wa_107: "Amsterdam",
  wa_108: "South of Italy",
  wa_112: "Venice",
  wa_113: "Geneva",
  wa_114: "Istanbul",
  wa_115: "Istanbul",
  wa_116: "Istanbul",
  wa_117: "The Bosphorus",
  wa_118: "England",
  wa_119: "Edinburgh",
  wa_12: "South Africa",
  wa_120: "Wales",
  wa_121: "England",
  wa_122: "England",
  wa_123: "Stratford-upon-Avon",
  wa_126: "Barcelona",
  wa_127: "Barcelona",
  wa_128: "Madrid",
  wa_129: "Barcelona",
  wa_13: "South Africa",
  wa_130: "Barcelona",
  wa_131: "Barcelona",
  wa_132: "Peru",
  wa_133: "Chile",
  wa_135: "New Orleans",
  wa_137: "Mexico",
  wa_138: "Mexico",
  wa_139: "Guatemala",
  wa_14: "Mali",
  wa_140: "Belize",
  wa_141: "The Vatican City State",
  wa_142: "Zambia and Zimbabwe",
  wa_143: "England",
  wa_144: "Tanzania",
  wa_145: "Cape Town",
  wa_146: "New Zealand",
  wa_147: "Japan",
  wa_148: "Israel",
  wa_149: "Italy",
  wa_15: "Mali",
  wa_150: "Italy",
  wa_151: "Indonesia",
  wa_152: "France",
  wa_156: "Antarctica",
  wa_157: "Berlin",
  wa_158: "Germany",
  wa_159: "Hohenschwangau",
  wa_16: "Ghana",
  wa_160: "Liberty Island",
  wa_161: "Delhi",
  wa_162: "Australia",
  wa_163: "Lithuania and Russia",
  wa_165: "Bolivia",
  wa_166: "South America",
  wa_167: "North Italy",
  wa_168: "South Africa and Lesotho",
  wa_169: "Ethiopia",
  wa_17: "Ethiopia",
  wa_172: "Mali",
  wa_173: "Hungary",
  wa_174: "Kenya",
  wa_175: "Botswana",
  wa_176: "Southern Africa",
  wa_177: "North Africa",
  wa_178: "Mongolia and China",
  wa_179: "South America",
  wa_18: "Ethiopia",
  wa_180: "Morroco",
  wa_181: "Pacific Ocean",
  wa_182: "Syria",
  wa_184: "Egypt",
  wa_186: "Indonesia",
  wa_187: "Thailand",
  wa_188: "Bhutan",
  wa_19: "Lhasa",
  wa_191: "Scotland",
  wa_192: "Italy",
  wa_194: "Indian Ocean",
  wa_195: "Pacific Ocean",
  wa_196: "Between Mongolia and Siberia",
  wa_197: "Off the coast of Chile",
  wa_198: "Kenya",
  wa_199: "Colorado",
  wa_2: "Algeria",
  wa_20: "China",
  wa_201: "Russia",
  wa_203: "Westminster",
  wa_204: "Rome",
  wa_205: "Florence",
  wa_206: "Uzbekistan",
  wa_207: "Hawaii",
  wa_209: "The Italian Riviera",
  wa_21: "China",
  wa_210: "Ho Chi Minh City",
  wa_211: "Poland",
  wa_212: "Between the Mediterranean Sea and Red Sea",
  wa_213: "Tivoli",
  wa_214: "Wiltshire",
  wa_215: "Haiti",
  wa_216: "Belize",
  wa_217: "Mumbai Harbour",
  wa_218: "United States of America",
  wa_219: "Germany",
  wa_23: "China",
  wa_24: "China",
  wa_26: "Beijing",
  wa_27: "Beijing",
  wa_28: "Beijing",
  wa_29: "Cambodia",
  wa_30: "Cambodia",
  wa_31: "Bangladesh",
  wa_33: "Bahrain",
  wa_35: "Afghanistan",
  wa_36: "Iran",
  wa_37: "Iran",
  wa_38: "Iran",
  wa_39: "Iran",
  wa_4: "Republic of the Congo",
  wa_40: "Indonesia",
  wa_41: "India",
  wa_42: "India",
  wa_43: "India",
  wa_44: "India",
  wa_45: "Delhi",
  wa_46: "Hong Kong",
  wa_47: "Hong Kong",
  wa_48: "Israel",
  wa_49: "Jerusalem",
  wa_5: "Ivory Coast",
  wa_50: "Jerusalem",
  wa_51: "Israel",
  wa_52: "Pakistan",
  wa_53: "Pakistan",
  wa_54: "Pakistan",
  wa_55: "Oman",
  wa_56: "Oman",
  wa_57: "Myanmar",
  wa_58: "Malaysia",
  wa_59: "Malaysia",
  wa_6: "Egypt",
  wa_60: "Kuala Lumpur",
  wa_61: "Kuwait",
  wa_62: "Jordan",
  wa_63: "Japan",
  wa_64: "Tokyo",
  wa_65: "Bangkok",
  wa_67: "Saudi Arabia",
  wa_68: "Saudi Arabia",
  wa_69: "Philippines",
  wa_7: "Egypt",
  wa_70: "Philippines",
  wa_71: "Croatia",
  wa_72: "Bulgaria",
  wa_73: "Bulgaria",
  wa_74: "Bulgaria",
  wa_75: "Bulgaria",
  wa_76: "Bulgaria",
  wa_77: "Belgium",
  wa_78: "Brussels",
  wa_79: "Belarus",
  wa_8: "Mount Sinai",
  wa_80: "Austria",
  wa_81: "Austria",
  wa_82: "Austria",
  wa_83: "Georgia",
  wa_84: "France",
  wa_85: "Paris",
  wa_89: "Prague",
  wa_9: "Tunisia",
  wa_90: "Bavaria",
  wa_91: "Germany",
  wa_92: "Frankfurt",
  wa_93: "Berlin",
  wa_94: "Ireland",
  wa_95: "Ireland",
  wa_96: "Dublin",
  wa_97: "Budapest",
  wa_98: "Athens",
  wa_99: "Tsarskoye Selo",
  wa_lgflag: "Namibia",
  ws_lgflag: "Samoa",
  x_102: "Brazil",
  x_104: "China",
  x_107: "Greece",
  x_108: "Guatemala",
  x_111: "Japan",
  x_124: "Guinea-Bissau",
  x_125: "Iraq",
  x_126: "Ireland",
  x_127: "Israel",
  x_128: "Norway",
  x_131: "Spain",
  x_133: "Switzerland",
  x_134: "Afghanistan",
  x_135: "Algeria",
  x_137: "Bosnia and Herzegovina",
  x_138: "Bulgaria",
  x_139: "Cambodia",
  x_143: "Georgia",
  x_144: "Hungary",
  x_146: "Iran",
  x_147: "Jamaica",
  x_148: "Lebanon",
  x_149: "Libya",
  x_150: "Malaysia",
  x_154: "South Korea",
  x_156: "Albania",
  x_158: "Belarus",
  x_159: "Colombia",
  x_160: "Croatia",
  x_162: "Ecuador",
  x_165: "Haiti",
  x_166: "Indonesia",
  x_168: "Kenya",
  x_169: "Latvia",
  x_170: "Liberia",
  x_171: "Liechtenstein",
  x_173: "Myanmar (Burma)",
  x_174: "Nepal",
  x_175: "Pakistan",
  x_177: "Paraguay",
  x_178: "Peru",
  x_179: "Qatar",
  x_182: "Senegal",
  x_183: "Serbia",
  x_184: "Seychelles",
  x_185: "Slovakia",
  x_186: "Somalia",
  x_188: "Syria",
  x_189: "Thailand",
  x_191: "Trinidad and Tobago",
  x_192: "Tunisia",
  x_194: "Uganda",
  x_196: "United Arab Emirates",
  x_197: "Uruguay",
  x_198: "Venezuela",
  x_199: "Angola",
  x_200: "Armenia",
  x_201: "Azerbaijan",
  x_202: "Bahrain",
  x_203: "Bangladesh",
  x_206: "Benin",
  x_207: "Bhutan",
  x_208: "Bolivia",
  x_209: "Botswana",
  x_210: "Brunei",
  x_211: "Burkina Faso",
  x_212: "Burundi",
  x_213: "Cameroon",
  x_214: "Cape Verde",
  x_215: "Central African Republic",
  x_216: "Chad",
  x_218: "Congo-Democratic Republic",
  x_219: "Congo-Republic",
  x_220: "Costa Rica",
  x_221: "Cote d'Ivoire",
  x_223: "Dominican Republic",
  x_224: "Equatorial Guinea",
  x_225: "Eritrea",
  x_226: "Estonia",
  x_227: "Ethiopia",
  x_228: "Fiji",
  x_229: "Gabon",
  x_230: "Ghana",
  x_232: "Guinea",
  x_233: "Guyana",
  x_234: "Honduras",
  x_235: "Kazakhstan",
  x_237: "Kyrgyzstan",
  x_238: "Laos",
  x_240: "Lithuania",
  x_241: "Macedonia",
  x_242: "Madagascar",
  x_243: "Malawi",
  x_245: "Mali",
  x_246: "Malta",
  x_248: "Mauritania",
  x_249: "Mauritius",
  x_251: "Moldova",
  x_252: "Montenegro",
  x_253: "Morocco",
  x_254: "Mozambique",
  x_255: "Namibia",
  x_256: "Nicaragua",
  x_257: "Niger",
  x_258: "Nigeria",
  x_259: "Oman",
  x_261: "Rwanda",
  x_266: "Sierra Leone",
  x_267: "Slovenia",
  x_268: "Solomon Islands",
  x_269: "Sri Lanka",
  x_271: "Suriname",
  x_272: "Swaziland",
  x_273: "Tajikistan",
  x_274: "Tanzania",
  x_275: "The Gambia",
  x_276: "Timor-Leste",
  x_277: "Togo",
  x_279: "Turkmenistan",
  x_281: "Uzbekistan",
  x_283: "Yemen",
  x_284: "Zambia",
  x_285: "Zimbabwe",
  x_286: "Djibouti",
  x_291: "Singapore",
  ym_lgflag: "Yemen",
  za_lgflag: "Zambia",
};

/* Vocabulary lookup (~level 26) */
var VocabAnswers = {
  abandonment: "desertion",
  abash: "embarrass",
  abatement: "reduction",
  abbey: "monastery",
  abbreviate: "shorten",
  abdomen: "belly",
  abduct: "kidnap",
  abduction: "kidnapping",
  abhor: "detest",
  abhorrence: "loathing",
  abhorrent: "detestable",
  abide: "tolerate",
  ability: "skill",
  abjection: "degradation",
  ablaze: "on fire",
  abnormality: "irregularity",
  abode: "home",
  abolish: "end",
  abomination: "loathing",
  aboriginal: "native",
  abortive: "unsuccessful",
  aboveboard: "straightforward",
  abrasion: "grinding",
  abrupt: "sudden",
  abruption: "breaking off",
  abruptly: "suddenly",
  absence: "not being present",
  absent: "missing",
  absolute: "pure",
  absolve: "pardon",
  abstain: "refrain",
  abstract: "theoretical",
  absurd: "ridiculous",
  absurdity: "ridiculousness",
  abundance: "plenty",
  abuse: "mistreat",
  abyss: "chasm",
  accelerate: "move faster",
  accent: "emphasize",
  accept: "receive",
  acceptable: "satisfactory",
  acceptance: "approval",
  accessible: "approachable",
  accidental: "by chance",
  acclaim: "praise",
  acclamation: "expression of approval",
  acclimate: "adapt to",
  acclimatize: "adapt",
  accolade: "honor",
  accommodating: "obliging",
  accompany: "escort",
  accomplice: "confederate",
  accomplished: "skillful",
  accord: "agreement",
  accordance: "conformity",
  accordant: "agreeing",
  accordingly: "consequently",
  accountable: "responsible",
  accredit: "recognize",
  accrue: "accumulate",
  accuracy: "exactness",
  accurate: "exact",
  accusation: "charge",
  accuse: "blame",
  accustom: "habituate",
  acerbity: "bitterness",
  ache: "pain",
  achieve: "accomplish",
  acknowledge: "admit",
  acknowledgment: "recognition",
  acquaint: "familiarize",
  acquire: "gain",
  acquit: "find not guilty",
  acrid: "bitter",
  acrophobia: "fear of heights",
  activate: "start",
  actual: "real",
  actualize: "make real",
  actually: "really",
  acute: "sharp",
  adamant: "unyielding",
  addiction: "dependence",
  addictive: "habit-forming",
  addition: "increase",
  adept: "skilled",
  adequate: "sufficient",
  adhere: "stick",
  adherent: "supporter",
  adhesion: "attachment",
  adieu: "farewell",
  adjacent: "neighboring",
  adjoin: "border",
  adjoining: "neighboring",
  adjourn: "recess",
  adjudicate: "judge",
  admeasure: "apportion",
  administer: "manage",
  administration: "management",
  admiration: "esteem",
  admire: "regard highly",
  admissible: "allowable",
  admonition: "warning",
  adolescent: "teenager",
  adorable: "endearing",
  adore: "love",
  adulation: "worship",
  adulterate: "make impure",
  advance: "move forward",
  advancement: "progress",
  advantageous: "beneficial",
  advent: "coming",
  adversary: "opponent",
  adverse: "unfavorable",
  adversity: "misfortune",
  advise: "counsel",
  advisement: "consideration",
  advocate: "recommend",
  aeronaut: "balloonist",
  aesthetic: "artistically beautiful",
  afar: "great distance",
  affair: "matter",
  affect: "act upon",
  affection: "fondness",
  affectionate: "loving",
  affiliation: "association",
  affliction: "distress",
  affluence: "wealth",
  affluent: "wealthy",
  affright: "terror",
  affront: "insult",
  aficionado: "avid follower",
  aforethought: "premeditated",
  aftermath: "consequence",
  aged: "old",
  agenda: "schedule",
  agglomeration: "cluster",
  aggrandize: "make greater",
  aggravate: "irritate",
  aggression: "hostility",
  aggrieve: "distress",
  aghast: "shocked",
  agile: "nimble",
  agility: "nimbleness",
  agitate: "disturb",
  agitator: "rabble-rouser",
  agonizing: "painful",
  agony: "pain",
  agriculture: "farming",
  aid: "help",
  aide: "assistant",
  ail: "trouble",
  alarmist: "scaremonger",
  alcove: "nook",
  alert: "watchful",
  alias: "false name",
  alibi: "excuse",
  alien: "foreign",
  alienate: "estrange",
  alike: "similar",
  allege: "assert",
  alleviate: "relieve",
  alleviation: "relief",
  alliance: "coalition",
  allot: "assign",
  allotment: "portion",
  allow: "permit",
  allowable: "permissible",
  allure: "entice",
  ally: "friend",
  almighty: "omnipotent",
  almost: "nearly",
  aloft: "in the air",
  alpha: "first",
  alter: "change",
  altercation: "dispute",
  alternative: "option",
  altitude: "height",
  altruism: "selflessness",
  alumnus: "graduate",
  amass: "accumulate",
  amateurish: "inexpert",
  amaze: "astound",
  amazing: "marvelous",
  ambient: "surrounding",
  ambiguity: "uncertainty",
  ambiguous: "uncertain",
  ambitious: "aspiring",
  ambivalence: "uncertainty",
  amble: "stroll",
  ambush: "lie in wait",
  amenable: "willing",
  amend: "improve",
  amiable: "friendly",
  amicable: "friendly",
  amiss: "faulty",
  amnesia: "forgetfulness",
  amnesty: "pardon",
  amok: "berserk",
  amorphous: "shapeless",
  amour: "love affair",
  amphitheater: "arena",
  ample: "abundant",
  amplify: "expand",
  amplitude: "largeness",
  amputate: "cut off",
  amuse: "entertain",
  anaconda: "boa",
  analgesic: "painkiller",
  analogous: "similar",
  analysis: "examination",
  analyze: "examine",
  anarchy: "lawlessness",
  anatomy: "structure",
  ancestry: "lineage",
  ancient: "old",
  andiron: "fireplace log holder",
  anemometer: "wind gauge",
  anger: "ire",
  angst: "anxiety",
  angular: "sharp-cornered",
  animated: "lively",
  animosity: "hatred",
  annex: "attach",
  annihilate: "destroy",
  annihilation: "destruction",
  annotate: "make comments about",
  annotation: "note",
  announce: "proclaim",
  annoy: "bother",
  annoyance: "irritation",
  annoying: "bothersome",
  annual: "yearly",
  annul: "void",
  annulment: "invalidation",
  anomaly: "irregularity",
  anonymity: "namelessness",
  anonymous: "unknown",
  answer: "reply",
  answerable: "responsible",
  antagonism: "hostility",
  antagonist: "opponent",
  antagonistic: "hostile",
  antagonize: "provoke",
  antecede: "go before",
  antemeridian: "before noon",
  anthology: "collection",
  anthropomorphic: "humanlike",
  anticipate: "expect",
  antidote: "remedy",
  antique: "old",
  antiquity: "ancientness",
  antithesis: "opposite",
  antonym: "opposite",
  anxiety: "worry",
  anxious: "worried",
  apathetic: "indifferent",
  apathy: "indifference",
  aperture: "opening",
  apex: "peak",
  apiarist: "beekeeper",
  apnea: "breathing difficulty",
  apocalyptic: "portending doom",
  appall: "dismay",
  appalling: "awful",
  apparel: "clothing",
  apparent: "evident",
  apparition: "ghost",
  appear: "seem",
  appease: "pacify",
  append: "attach",
  applaud: "clap",
  applause: "clapping",
  appoint: "designate",
  apportion: "allot",
  appraisal: "assessment",
  appraise: "evaluate",
  appreciate: "value",
  appreciation: "gratitude",
  appreciative: "grateful",
  apprehend: "arrest",
  apprehension: "dread",
  apprehensive: "fearful",
  approach: "go near",
  approachable: "accessible",
  appropriate: "suitable",
  approximate: "near",
  aptitude: "talent",
  aqua: "water",
  aqueous: "watery",
  arbitrary: "random",
  arbitrator: "judge",
  archaic: "antiquated",
  archipelago: "group of islands",
  archives: "records",
  arctic: "frigid",
  argue: "dispute",
  argumentation: "debate",
  arid: "dry",
  aristocracy: "nobility",
  aristocrat: "patrician",
  armada: "fleet",
  armament: "weaponry",
  armistice: "truce",
  armory: "arsenal",
  aroma: "fragrance",
  aromatic: "fragrant",
  arouse: "excite",
  arrogance: "haughtiness",
  arrogant: "haughty",
  arsenal: "armory",
  arson: "malicious fire-setting",
  articulate: "well-spoken",
  artillery: "heavy weapons",
  artisan: "craftsman",
  ascend: "go up",
  ascendancy: "dominance",
  ascension: "rising",
  ascribe: "attribute",
  ashen: "pale",
  ashore: "on land",
  asphyxiate: "suffocate",
  aspiration: "ambition",
  assail: "attack",
  assailant: "attacker",
  assassinate: "murder",
  assassination: "murder",
  assault: "attack",
  assert: "declare",
  assess: "valuate",
  assist: "help",
  assistant: "helper",
  associate: "companion",
  assortment: "variety",
  assuming: "presumptuous",
  assure: "guarantee",
  assuredly: "certainly",
  astonish: "amaze",
  astonishing: "extraordinary",
  astounding: "incredible",
  astral: "pertaining to stars",
  astronomer: "stargazer",
  asylum: "sanctuary",
  asymmetric: "uneven",
  asymmetry: "imbalance",
  atone: "make amends",
  atrocious: "abominable",
  atrocity: "brutal inhumanity",
  attach: "connect",
  attain: "achieve",
  attempt: "try",
  attendant: "accompanying",
  attention: "notice",
  attire: "clothes",
  attorney: "lawyer",
  attribute: "characteristic",
  atypical: "irregular",
  auburn: "reddish-brown",
  audacious: "daring",
  audacity: "boldness",
  audible: "loud enough to hear",
  audio: "sound",
  audition: "tryout",
  aura: "atmosphere",
  authenticate: "confirm",
  authenticity: "genuineness",
  author: "writer",
  authoritarian: "dictatorial",
  authoritative: "official",
  autogenous: "self-generated",
  automaton: "robot",
  automobile: "car",
  autonomous: "self-governing",
  autonomy: "self-government",
  autosuggestion: "self-hypnosis",
  autumn: "fall",
  auxiliary: "additional",
  avail: "benefit",
  avenge: "retaliate",
  averse: "reluctant",
  avert: "turn away",
  avian: "birdlike",
  aviary: "bird sanctuary",
  aviate: "fly",
  aviation: "plane-flying",
  avid: "eager",
  avoid: "shun",
  avoidance: "keeping away",
  avow: "declare",
  awash: "flooded",
  awesome: "amazing",
  awkward: "clumsy",
  axiom: "self-evident truth",
  azure: "sky-blue",
  babble: "chatter",
  baby: "infant",
  bachelorette: "unmarried woman",
  backbone: "spine",
  backbreaking: "laborious",
  backing: "support",
  backpedal: "retreat",
  backwash: "aftermath",
  badger: "pester",
  badly: "poorly",
  baffle: "perplex",
  baggy: "loose-fitting",
  balance: "equilibrium",
  bald: "hairless",
  balderdash: "nonsense",
  bale: "bundle",
  baleen: "whalebone",
  ballad: "song",
  balm: "ointment",
  bambino: "toddler",
  bamboozle: "dupe",
  ban: "forbid",
  bangle: "bracelet",
  banish: "exile",
  banister: "handrail",
  banner: "flag",
  banquet: "feast",
  baptize: "christen",
  barbarian: "savage",
  barbaric: "uncivilized",
  barbarism: "savagery",
  barbarity: "inhumanity",
  barbarous: "uncivilized",
  barber: "haircutter",
  barely: "scarcely",
  bargain: "deal",
  barge: "flatboat",
  barrage: "bombardment",
  barren: "sterile",
  barricade: "obstruction",
  barrier: "obstacle",
  barter: "trade",
  barterer: "trader",
  basement: "cellar",
  bashful: "shy",
  basin: "washbowl",
  basis: "foundation",
  bask: "luxuriate",
  bassinet: "baby's bed",
  bathe: "wash",
  batty: "loony",
  bawl: "cry",
  bazaar: "market",
  beak: "bill",
  beam: "ray",
  beast: "animal",
  beastly: "abominable",
  beckon: "summon",
  becloud: "obscure",
  bedevil: "torment",
  bedlam: "madhouse",
  beefeater: "yeoman",
  before: "previously",
  beforehand: "in advance",
  befuddle: "confuse",
  begrudge: "envy",
  behave: "act",
  behavior: "conduct",
  behemoth: "something huge",
  behest: "command",
  behold: "see",
  being: "existence",
  belated: "delayed",
  belfry: "bell tower",
  belief: "opinion",
  belittle: "disparage",
  belligerence: "hostility",
  belligerent: "quarrelsome",
  belligerently: "hostilely",
  bellow: "roar",
  belly: "abdomen",
  beloved: "dear",
  benchmark: "standard",
  benediction: "blessing",
  benefactor: "patron",
  beneficial: "helpful",
  benefit: "advantage",
  bent: "crooked",
  bequeath: "hand down",
  berate: "scold",
  bereavement: "mourning",
  berserk: "crazed",
  besiege: "beleaguer",
  bestial: "animal-like",
  bestow: "confer",
  bestride: "straddle",
  bet: "wager",
  betrothal: "engagement",
  betrothed: "engaged",
  betwixt: "between",
  beverage: "drink",
  bewilder: "confuse",
  bewilderment: "confusion",
  bias: "prejudice",
  biased: "prejudiced",
  bibliophile: "book lover",
  bibliopole: "rare book seller",
  bibliotheca: "library",
  bicker: "squabble",
  biff: "whack",
  bigotry: "prejudice",
  bilateral: "two-sided",
  billfold: "wallet",
  bin: "storage receptacle",
  bind: "tie",
  bipartisan: "two-party",
  birthright: "privilege",
  bisect: "divide",
  bison: "buffalo",
  bisque: "soup",
  blab: "chatter",
  blabber: "chatter",
  blackmail: "extortion",
  blame: "fault",
  blandishment: "flattery",
  blaspheme: "curse",
  blasphemous: "profane",
  blasphemy: "sacrilege",
  blatant: "obvious",
  blather: "babble",
  blaze: "flame",
  bleak: "desolate",
  bleary: "tired",
  blemish: "flaw",
  bliss: "happiness",
  blissful: "happy",
  blizzard: "snowstorm",
  bloat: "inflate",
  blockade: "obstruction",
  blockage: "obstruction",
  blockhead: "numskull",
  bloodcurdling: "horrific",
  bloodline: "lineage",
  bloodshed: "slaughter",
  bloom: "blossom",
  blossom: "flower",
  blot: "stain",
  blotch: "spot",
  blubber: "fat",
  bludgeon: "club",
  blueprint: "design",
  blush: "redden",
  bluster: "swagger",
  boast: "brag",
  bodily: "physical",
  bog: "swamp",
  bogus: "sham",
  bohemian: "unconventional",
  boisterous: "noisy",
  bolero: "Spanish dance",
  bombard: "barrage",
  bombardier: "artilleryman",
  bombardment: "barrage",
  bombastic: "pompous",
  bonanza: "windfall",
  bond: "tie",
  bonkers: "nutty",
  booking: "reservation",
  bookish: "studious",
  boondocks: "hinterland",
  boor: "uncultured person",
  boost: "raise",
  booster: "supporter",
  booth: "stall",
  booty: "plunder",
  border: "boundary",
  borderline: "marginal",
  boring: "tiresome",
  borough: "town",
  borscht: "beet soup",
  botch: "bungle",
  bother: "annoy",
  bothersome: "annoying",
  bottleneck: "obstruction",
  bough: "limb",
  bouillon: "broth",
  boulder: "large rock",
  boulevard: "avenue",
  boundary: "limit",
  boundless: "vast",
  bounteous: "generous",
  bounty: "reward",
  boutique: "shop",
  bovine: "cow-like",
  bowman: "archer",
  brag: "boast",
  braggart: "boaster",
  braid: "interweave",
  brainwash: "indoctrinate",
  brandish: "wave around menacingly",
  brash: "audacious",
  brassy: "impudent",
  brave: "courageous",
  bravery: "courage",
  brawl: "fight",
  brawn: "muscle",
  brawny: "muscular",
  brazen: "shameless",
  breach: "violation",
  breadth: "width",
  breakwater: "jetty",
  breathtaking: "astonishing",
  breezy: "airy",
  bribe: "corrupt payoff",
  brief: "concise",
  briefly: "concisely",
  brine: "salt water",
  brink: "verge",
  briny: "salty",
  britches: "trousers",
  brittle: "fragile",
  broad: "wide",
  broadcast: "transmit",
  brooch: "pin",
  broth: "thin soup",
  brouhaha: "uproar",
  brow: "forehead",
  browse: "skim",
  brunt: "impact",
  brutal: "cruel",
  brutality: "savagery",
  brute: "beast",
  bubbly: "fizzy",
  buccaneer: "pirate",
  buddy: "pal",
  budge: "move",
  buffer: "cushion",
  buffoon: "clown",
  buffoonery: "clowning",
  bulk: "mass",
  bullheaded: "obstinate",
  bully: "intimidate",
  bummer: "disappointment",
  bumpkin: "yokel",
  bun: "roll",
  bundle: "package",
  bungalow: "cottage",
  bungle: "botch",
  bunker: "dugout",
  burden: "load",
  burdensome: "oppressive",
  bureau: "dresser",
  burlap: "coarse fabric",
  burly: "husky",
  burnish: "polish",
  burp: "belch",
  burro: "donkey",
  burst: "break open",
  bush: "shrub",
  busybody: "snoop",
  bygone: "past",
  bystander: "onlooker",
  byway: "side road",
  byword: "common saying",
  cabana: "bathhouse",
  cabaret: "nightclub",
  cable: "wire",
  cache: "hoard",
  cad: "scoundrel",
  cagey: "shrewd",
  calamari: "squid",
  calamity: "disaster",
  calculate: "compute",
  calculating: "scheming",
  calligraphy: "artistic handwriting",
  callous: "hardened",
  camaraderie: "good-fellowship",
  camouflage: "disguise",
  cancel: "call off",
  candid: "frank",
  candor: "frankness",
  canine: "doglike",
  cannibal: "man-eater",
  canny: "shrewd",
  cantankerous: "bad-tempered",
  canyon: "ravine",
  caper: "prank",
  capsize: "overturn",
  captivate: "fascinate",
  captive: "prisoner",
  carafe: "wide-mouthed bottle",
  carcass: "corpse",
  carcinogenic: "cancer-causing",
  cardiologist: "heart specialist",
  carefree: "unworried",
  cargo: "freight",
  caribou: "reindeer",
  caricature: "parody",
  carnage: "slaughter",
  carnival: "festival",
  carnivore: "meat-eater",
  carnivorous: "meat-eating",
  carping: "fault-finding",
  carrier: "bearer",
  cartilage: "gristle",
  cartographer: "map-maker",
  cartography: "map-making",
  cascade: "waterfall",
  cask: "barrel",
  casket: "coffin",
  castle: "fortress",
  cataclysm: "disaster",
  catastrophe: "disaster",
  catastrophic: "disastrous",
  catching: "contagious",
  cater: "provide",
  catholicity: "universality",
  catnap: "snooze",
  cease: "stop",
  ceaseless: "constant",
  cede: "yield",
  celebrity: "famous person",
  celestial: "heavenly",
  cellar: "basement",
  cemetery: "graveyard",
  central: "middle",
  certain: "sure",
  certainly: "for sure",
  certainty: "sure thing",
  cessation: "stop",
  chance: "opportunity",
  chancy: "uncertain",
  change: "alter",
  chaos: "disorder",
  characterize: "describe",
  charisma: "personal appeal",
  charitable: "benevolent",
  charlatan: "fraud",
  charm: "fascinate",
  charmed: "delighted",
  charming: "delightful",
  chart: "map",
  chase: "pursue",
  chasm: "abyss",
  chassis: "frame",
  chasten: "discipline",
  chatty: "talkative",
  cheap: "inexpensive",
  cheapskate: "miser",
  checkup: "medical examination",
  cheeky: "impudent",
  cheerful: "in good spirits",
  chef: "cook",
  cherish: "treasure",
  cherubic: "angelic",
  chic: "stylish",
  chick: "young bird",
  chickenhearted: "cowardly",
  chide: "scold",
  chiefly: "principally",
  chiffon: "sheer fabric",
  chilly: "cool",
  chip: "fragment",
  chitchat: "small talk",
  chivalric: "gallant",
  chivalrous: "gallant",
  chivalry: "gallantry",
  choke: "strangle",
  chomp: "bite",
  choosy: "particular",
  chopper: "helicopter",
  chore: "task",
  choreography: "dance arrangement",
  chorizo: "spicy sausage",
  chortle: "laugh quietly",
  chow: "food",
  chronic: "habitual",
  chronicle: "account",
  chronometer: "clock",
  chuckle: "laugh softly",
  chum: "buddy",
  churn: "agitate",
  cider: "apple juice",
  cinema: "movies",
  circuitous: "roundabout",
  circular: "round",
  circulate: "spread",
  circumfuse: "spread around",
  circumnavigate: "sail around",
  circumvent: "get around",
  citadel: "fortress",
  cite: "quote",
  civility: "politeness",
  clamber: "scramble",
  clammy: "damp",
  clamorous: "noisy",
  clan: "tribe",
  clarify: "make clear",
  clarity: "clearness",
  classy: "elegant",
  clavicle: "collarbone",
  clear: "transparent",
  cleave: "split",
  cleft: "split",
  cliche: "trite saying",
  client: "customer",
  climate: "weather",
  clobber: "bash",
  clod: "lump",
  clone: "duplicate",
  clownish: "zany",
  clue: "hint",
  clumsy: "awkward",
  clunk: "thump",
  clunky: "ungainly",
  cluster: "bunch",
  clutch: "grasp",
  clutter: "disorder",
  coagulate: "clot",
  coalition: "alliance",
  coax: "urge",
  cobbler: "shoemaker",
  coccyx: "tail bone",
  cocksure: "overconfident",
  cocky: "self-confident",
  coddle: "pamper",
  coerce: "compel",
  coffin: "casket",
  cognition: "knowledge",
  cohabit: "live together",
  cohere: "stick together",
  cohort: "associate",
  coinage: "minting",
  colander: "strainer",
  collaborate: "work together",
  collapse: "crumple",
  collate: "arrange in order",
  colleague: "associate",
  collect: "accumulate",
  colonist: "settler",
  colonization: "settlement",
  colossal: "enormous",
  colossus: "giant",
  column: "pillar",
  comatose: "unconscious",
  combat: "battle",
  combatant: "fighter",
  combine: "merge",
  comeback: "retort",
  comedy: "funny show",
  comic: "comedian",
  comical: "funny",
  commence: "begin",
  commencement: "beginning",
  commend: "praise",
  comment: "remark",
  commerce: "business",
  commingle: "blend",
  commiserate: "sympathize",
  commiseration: "pity",
  commode: "toilet",
  commodity: "product",
  commodore: "naval captain",
  commonplace: "ordinary",
  commonwealth: "state",
  commotion: "disturbance",
  communion: "fellowship",
  compare: "liken",
  compassion: "sympathy",
  compassionate: "sympathetic",
  compel: "force",
  competence: "ability",
  competency: "ability",
  compile: "put together",
  complacence: "self-satisfaction",
  complacency: "self-satisfaction",
  completely: "totally",
  compliant: "yielding",
  complimentary: "free",
  composite: "made of different parts",
  composure: "calmness",
  comprehend: "understand",
  comprehension: "understanding",
  comprehensive: "wide-reaching",
  compress: "squeeze",
  comprise: "consist of",
  compulsory: "obligatory",
  concave: "curving inward",
  conceal: "hide",
  concede: "yield",
  conceit: "vanity",
  concentrate: "focus",
  concentric: "with a common center",
  concerned: "anxious",
  concise: "brief",
  concoct: "devise",
  concur: "agree",
  concurrence: "agreement",
  concurrent: "simultaneous",
  condescending: "patronizing",
  condole: "express sympathy",
  condolence: "sympathy",
  condone: "tolerate",
  conducive: "helpful",
  confectioner: "candymaker",
  confederate: "ally",
  confederation: "alliance",
  confer: "bestow",
  conference: "meeting",
  confide: "trust",
  confine: "limit",
  confirm: "corroborate",
  confiscate: "seize",
  confluence: "flowing together",
  conform: "comply",
  confound: "bewilder",
  confounded: "perplexed",
  confront: "face",
  confuse: "perplex",
  congenial: "agreeable",
  congest: "clog",
  congestion: "crowding",
  congregate: "assemble",
  coniferous: "bearing cones",
  conjecture: "guess",
  conjoin: "unite",
  conjunctivitis: "pinkeye",
  conjure: "evoke",
  conjurer: "magician",
  connect: "join",
  connive: "scheme",
  connoisseur: "food or art expert",
  conquer: "defeat",
  conscious: "aware",
  consecrate: "sanctify",
  consensus: "agreement",
  consent: "agree",
  consequence: "result",
  consequently: "accordingly",
  conservationist: "environmentalist",
  conserve: "maintain without loss",
  considerate: "thoughtful",
  consign: "entrust",
  consolation: "comfort",
  console: "comfort",
  consolidate: "combine",
  conspicuous: "noticeable",
  conspiracy: "plot",
  conspire: "plot",
  constable: "lawman",
  constant: "steadfast",
  constantly: "always",
  construct: "build",
  consultant: "advisor",
  consummate: "complete",
  contagious: "communicable",
  contain: "hold",
  contaminate: "pollute",
  contemplate: "ponder",
  contempt: "disdain",
  contemptible: "despicable",
  contemptuous: "disdainful",
  contend: "struggle",
  contented: "satisfied",
  context: "setting",
  continuous: "uninterrupted",
  contraction: "shortening",
  contradiction: "inconsistency",
  contradictory: "inconsistent",
  contraption: "gadget",
  contribute: "give",
  contrive: "devise",
  controversy: "dispute",
  convalescence: "recuperation",
  convene: "assemble",
  convent: "nunnery",
  converge: "meet",
  convert: "transform",
  conveyance: "transfer",
  convince: "persuade",
  convoy: "escort",
  convulsion: "paroxysm",
  convulsive: "spasmodic",
  cooperate: "work together",
  cope: "manage",
  cordial: "friendly",
  cordiality: "friendliness",
  cornerstone: "foundation",
  cornucopia: "horn of plenty",
  coronation: "crowning",
  corpse: "dead body",
  corridor: "passageway",
  corroborate: "confirm",
  corroboration: "confirming evidence",
  corrode: "wear away",
  corrosive: "eating away",
  corrugated: "ridged",
  corset: "girdle",
  cosmetic: "decorative",
  cosmopolitan: "worldly",
  cosmopolite: "worldly person",
  costly: "expensive",
  cottage: "small house",
  couch: "sofa",
  council: "assembly",
  counsel: "advise",
  counselor: "adviser",
  counteract: "neutralize",
  counterfeit: "fake",
  counterfeiter: "forger",
  coup: "takeover",
  courageous: "brave",
  courier: "messenger",
  courtesy: "politeness",
  courtly: "elegant",
  courtship: "wooing",
  cove: "bay",
  covenant: "agreement",
  covert: "secret",
  covet: "desire",
  cower: "cringe",
  coy: "shy",
  crackpot: "eccentric",
  craftsman: "artisan",
  crafty: "cunning",
  cranium: "skull",
  cranky: "irritable",
  cranny: "crevice",
  cravat: "scarf necktie",
  crave: "desire",
  craze: "mania",
  crease: "wrinkle",
  credence: "belief",
  credibility: "believableness",
  credible: "believable",
  creed: "belief",
  creek: "brook",
  creep: "crawl",
  crest: "top",
  crestfallen: "dejected",
  crevasse: "fissure",
  crevice: "crack",
  criminalize: "outlaw",
  criminate: "accuse",
  cringe: "cower",
  critique: "review",
  crochet: "needlework",
  crone: "hag",
  crony: "pal",
  crossbeam: "girder",
  crossbreed: "hybrid",
  crossroad: "intersection",
  crucial: "decisive",
  crud: "filth",
  crumble: "disintegrate",
  crummy: "lousy",
  crusade: "campaign",
  crusty: "gruff",
  crypt: "vault",
  cryptic: "secret",
  cryptogram: "coded message",
  cryptography: "secret writing",
  cudgel: "bludgeon",
  cuisine: "cooking style",
  culinary: "cooking-related",
  culprit: "offender",
  cultivated: "refined",
  cultured: "refined",
  cumbersome: "unwieldy",
  cumulonimbus: "thundercloud",
  cuneiform: "wedge-shaped",
  cunning: "craftiness",
  curator: "custodian",
  curio: "oddity",
  curious: "inquisitive",
  currency: "money",
  curse: "swear",
  curt: "terse",
  curtail: "shorten",
  curtain: "drape",
  curvaceous: "voluptuous",
  curvature: "bending",
  cusp: "tip",
  custodian: "keeper",
  custody: "care",
  customary: "usual",
  cutlet: "thin slice of meat",
  cutthroat: "ruthless",
  cutting: "sharp",
  cyclone: "tornado",
  czar: "Russian emperor",
  czarina: "Russian empress",
  dacha: "Russian country house",
  daft: "foolish",
  dainty: "exquisite",
  dally: "waste time",
  damage: "harm",
  dame: "woman",
  damp: "moist",
  damsel: "maiden",
  dangle: "swing",
  dank: "clammy",
  dapper: "stylishly dressed",
  dare: "challenge",
  darling: "beloved",
  dastard: "treacherous coward",
  data: "information",
  datum: "fact",
  daub: "smear",
  daunt: "intimidate",
  dawdle: "linger",
  dawn: "sunrise",
  daybreak: "dawn",
  daze: "stun",
  deadpan: "expressionless",
  dealership: "franchise",
  dearly: "fondly",
  debilitate: "weaken",
  debility: "weakness",
  debonair: "suave",
  debris: "rubbish",
  debut: "first appearance",
  decade: "ten years",
  decapitate: "behead",
  deceased: "dead",
  deceit: "trickery",
  deceitful: "dishonest",
  deceive: "mislead",
  decelerate: "slow down",
  deceptive: "misleading",
  decimate: "destroy",
  decompose: "rot",
  decree: "edict",
  decrepit: "worn out",
  dedication: "commitment",
  deduce: "infer",
  deduct: "subtract",
  deface: "mar",
  defalcation: "embezzlement",
  defamation: "slander",
  defame: "slander",
  defect: "flaw",
  defend: "guard",
  defiant: "rebellious",
  deficiency: "inadequacy",
  deficient: "lacking",
  deficit: "shortage",
  deflect: "turn aside",
  defoliate: "take leaves off",
  deform: "contort",
  deformed: "misshapen",
  defraud: "cheat",
  defy: "challenge",
  degenerate: "become worse",
  degeneration: "decline",
  degraded: "lowered in value",
  dehydrate: "dry up",
  deign: "lower oneself",
  deity: "god or goddess",
  deject: "dishearten",
  dejection: "melancholy",
  delectable: "scrumptious",
  delegate: "assign",
  delete: "erase",
  deletion: "erasing",
  deliberate: "intentional",
  deliberately: "on purpose",
  deliberation: "careful consideration",
  delight: "joy",
  delirious: "irrational",
  delirium: "madness",
  deliverance: "rescue",
  deltoid: "shoulder muscle",
  delude: "deceive",
  deluge: "flood",
  delusion: "false belief",
  delusive: "misleading",
  deluxe: "high quality",
  demanding: "difficult",
  demean: "degrade",
  demeanor: "behavior",
  demented: "insane",
  demerit: "mark of misconduct",
  demilitarize: "disarm",
  demise: "death",
  demobilize: "disband",
  demolish: "destroy",
  demolition: "destruction",
  demon: "devil",
  demonstrate: "show",
  demoralize: "discourage",
  demote: "lower in rank",
  den: "lair",
  denounce: "criticize",
  dense: "thick",
  denude: "strip",
  denunciation: "condemnation",
  deny: "contradict",
  depart: "leave",
  dependable: "trustworthy",
  dependence: "reliance",
  depict: "portray",
  depletion: "emptying",
  deplorable: "disgraceful",
  deport: "exile",
  deportee: "exile",
  deposition: "testimony",
  depository: "storehouse",
  depravity: "immorality",
  depress: "sadden",
  depressed: "sad",
  depressing: "dreary",
  deprive: "take away",
  deprived: "needy",
  deranged: "crazy",
  dermatologist: "skin doctor",
  dermatosis: "skin disease",
  desegregate: "mix",
  desertion: "abandonment",
  deserving: "worthy",
  design: "plan",
  designate: "specify",
  designing: "scheming",
  desirable: "pleasing",
  desire: "want",
  desist: "cease",
  desolate: "barren",
  desperado: "outlaw",
  despise: "scorn",
  despondence: "sadness",
  despondent: "disheartened",
  destiny: "fate",
  destitute: "poor",
  destitution: "poverty",
  destruction: "ruin",
  detached: "separate",
  detain: "hold in custody",
  detention: "custody",
  deter: "prevent",
  detergent: "soap",
  deteriorate: "grow worse",
  deterioration: "worsening",
  determinate: "fixed",
  deterrence: "disincentive",
  deterrent: "disincentive",
  detest: "hate",
  detestable: "abhorrent",
  detestation: "loathing",
  detonate: "explode",
  detonation: "explosion",
  detract: "take away",
  detractor: "critic",
  detrimental: "damaging",
  deuce: "two",
  devastate: "ravage",
  devilish: "diabolical",
  devilry: "mischief",
  devious: "sneaky",
  devise: "invent",
  devoid: "empty",
  devote: "dedicate",
  devotee: "follower",
  devour: "consume",
  devout: "religious",
  devoutly: "piously",
  dexterous: "skillful",
  dextral: "right-handed",
  diabolical: "devilish",
  dialogue: "conversation",
  diary: "journal",
  dicey: "unpredictable",
  dichromatic: "two-colored",
  dictatorial: "overbearing",
  dictatorship: "rule by one person",
  diction: "wording",
  differentiate: "tell apart",
  dignify: "honor",
  dignitary: "important person",
  dignity: "self-respect",
  dilapidate: "decay",
  dilapidated: "broken-down",
  dilate: "expand",
  dilation: "expansion",
  dilemma: "difficult choice",
  diligence: "effort",
  diligent: "industrious",
  dillydally: "dawdle",
  dilute: "thin",
  diminish: "lessen",
  dimwit: "dunce",
  dingo: "wild dog",
  dingy: "shabby",
  diplomatic: "tactful",
  dire: "dismal",
  director: "manager",
  disability: "handicap",
  disable: "incapacitate",
  disadvantageous: "unfavorable",
  disaffect: "alienate",
  disagree: "differ",
  disagreeable: "unpleasant",
  disappear: "vanish",
  disappointing: "unsatisfying",
  disarray: "confusion",
  disastrous: "ill-fated",
  disavowal: "denial",
  disband: "scatter",
  discard: "reject",
  discharge: "release",
  disclose: "reveal",
  discombobulated: "bewildered",
  discomfiture: "frustration",
  discontinue: "stop",
  discord: "conflict",
  discordant: "inharmonic",
  discourteous: "rude",
  discourtesy: "rudeness",
  discrepancy: "variance",
  discrete: "separate",
  discretion: "prudence",
  discrimination: "prejudice",
  disdain: "scorn",
  disdainful: "contemptuous",
  disembark: "go ashore",
  disembowel: "eviscerate",
  disengage: "detach",
  disfigure: "mar",
  disgorge: "vomit",
  disgrace: "shame",
  disgruntled: "unhappy",
  disgusting: "repugnant",
  dishearten: "demoralize",
  dishevel: "tousle",
  disheveled: "unkempt",
  dishonor: "shame",
  dishonorable: "shameful",
  disinclination: "reluctance",
  disinfectant: "germicide",
  disintegration: "breakdown",
  disinterested: "impartial",
  dismal: "gloomy",
  dismantle: "take apart",
  dismay: "consternation",
  disobedient: "noncompliant",
  disorder: "confusion",
  disperse: "scatter",
  dispersion: "scattering",
  dispirited: "listless",
  displace: "remove",
  display: "show",
  dispose: "throw away",
  disposed: "inclined",
  disposition: "temperament",
  disproof: "refutation",
  disprove: "refute",
  disputation: "controversy",
  dispute: "argument",
  disregard: "ignore",
  disrobe: "undress",
  dissect: "take apart",
  dissertation: "thesis",
  dissimilar: "unlike",
  dissipate: "scatter",
  dissolve: "liquefy",
  dissonance: "discord",
  dissuade: "deter",
  distant: "remote",
  distasteful: "offensive",
  distinct: "well-defined",
  distinctly: "clearly",
  distinguished: "illustrious",
  distort: "twist",
  distraught: "agitated",
  distress: "suffering",
  distressful: "worrisome",
  distribution: "arrangement",
  district: "region",
  distrustful: "suspicious",
  ditch: "trench",
  ditty: "song",
  dive: "plunge",
  diverse: "various",
  diversity: "variety",
  divert: "redirect",
  divine: "godlike",
  division: "separation",
  divulge: "reveal",
  dizzy: "lightheaded",
  docile: "obedient",
  dock: "pier",
  dodge: "evade",
  doe: "female deer",
  doggedness: "persistence",
  dogmatic: "opinionated",
  doldrums: "stagnation",
  dollop: "portion",
  dolt: "dullard",
  domesticate: "tame",
  domination: "supremacy",
  domineering: "imperious",
  dominion: "sovereignty",
  don: "put on",
  donation: "contribution",
  dormancy: "temporary inactivity",
  dormant: "inactive",
  dorsal: "back",
  doting: "excessively fond",
  doubt: "uncertainty",
  doubtful: "questionable",
  doughy: "pasty",
  douse: "drench",
  dowdy: "frumpy",
  downcast: "gloomy",
  downfall: "ruin",
  downhearted: "dejected",
  downpour: "deluge",
  downtrodden: "oppressed",
  downy: "soft",
  doze: "sleep lightly",
  dozen: "twelve",
  dozy: "sleepy",
  drab: "somber",
  drag: "haul",
  drama: "play",
  dramatist: "playwright",
  drape: "curtain",
  drastic: "severe",
  drawback: "disadvantage",
  dread: "fear",
  dreadful: "terrible",
  dregs: "sediment",
  drench: "soak",
  dribble: "trickle",
  driblet: "drop",
  drifter: "vagabond",
  drivel: "slobber",
  drool: "drivel",
  droop: "sag",
  droopy: "sagging",
  drought: "dryness",
  drover: "herdsman",
  drowsy: "sleepy",
  druggist: "pharmacist",
  drugstore: "pharmacy",
  drunk: "intoxicated",
  dub: "nickname",
  dubious: "questionable",
  duct: "tube",
  dumbfound: "amaze",
  dumbstruck: "flabbergasted",
  dunce: "simpleton",
  dunk: "dip",
  dupe: "fool",
  duplex: "twofold",
  duplicate: "copy",
  durable: "lasting",
  duress: "pressure",
  dusk: "twilight",
  duteous: "obedient",
  dutiful: "obedient",
  dweeb: "nerd",
  dwell: "inhabit",
  dweller: "inhabitant",
  dwelling: "home",
  dwindle: "shrink",
  dynamism: "vigor",
  eager: "keen",
  earnest: "sincere",
  earsplitting: "loud",
  earthenware: "pottery",
  earthy: "crude",
  easygoing: "relaxed",
  eavesdrop: "listen in",
  eccentric: "odd",
  eccentricity: "oddness",
  echo: "resound",
  economical: "frugal",
  economize: "conserve money",
  ecstasy: "extreme joy",
  edging: "border",
  edgy: "tense",
  edible: "fit to eat",
  edict: "decree",
  educator: "teacher",
  eerie: "strange",
  effectuate: "bring about",
  effeminacy: "womanishness",
  effervescent: "bubbling",
  efficacy: "effectiveness",
  efflorescence: "blossoming",
  egoism: "self-centeredness",
  egoistic: "self-centered",
  egotism: "self-importance",
  egotist: "conceited person",
  egotistic: "self-loving",
  egotistical: "vain",
  elapse: "pass",
  elastic: "springy",
  elasticity: "springiness",
  elated: "jubilant",
  elder: "senior",
  elect: "choose by vote",
  elector: "voter",
  electorate: "voters",
  elegant: "graceful",
  elegy: "mournful poem",
  elementary: "simple",
  elephantine: "huge",
  elevate: "raise",
  elevated: "high",
  elevation: "altitude",
  eliminate: "remove",
  elixir: "potion",
  ellipse: "oval",
  elongate: "lengthen",
  eloquent: "well-spoken",
  elude: "evade",
  emancipation: "freedom",
  embargo: "prohibition",
  embark: "go on board",
  embellishment: "decoration",
  embezzle: "steal",
  embezzlement: "theft",
  emblem: "symbol",
  emblematic: "symbolic",
  embodiment: "incarnation",
  embody: "personify",
  embrace: "hug",
  embryonic: "undeveloped",
  emerge: "come out",
  emissary: "messenger",
  emit: "give out",
  emotion: "feeling",
  empathetic: "understanding",
  empathize: "understand",
  emphasis: "special importance",
  emphasize: "stress",
  empire: "realm",
  employee: "worker",
  employment: "work",
  emporium: "store",
  empower: "enable",
  empty: "vacant",
  emulate: "imitate",
  enable: "empower",
  enchant: "bewitch",
  enchanting: "bewitching",
  enchantment: "spell",
  encircle: "surround",
  encompass: "include",
  encroach: "intrude",
  encroachment: "intrusion",
  endanger: "jeopardize",
  endorsement: "approval",
  endurable: "tolerable",
  endure: "last",
  enduring: "lasting",
  enfeeble: "weaken",
  engorge: "glut",
  engrave: "carve",
  engross: "absorb",
  engrossing: "fascinating",
  enhance: "heighten",
  enjoyment: "pleasure",
  enlarge: "expand",
  enlargement: "expansion",
  enlighten: "instruct",
  ennoble: "dignify",
  enroll: "register",
  enrollment: "registration",
  enshroud: "cover",
  ensnare: "trap",
  ensue: "follow",
  ensure: "make certain",
  entail: "involve",
  enterprise: "undertaking",
  enterprising: "venturesome",
  entertaining: "amusing",
  enthrall: "captivate",
  entice: "tempt",
  entire: "complete",
  entirely: "completely",
  entirety: "completeness",
  entity: "being",
  entomb: "bury",
  entourage: "attendants",
  enunciate: "pronounce",
  envelop: "surround",
  environmentalist: "conservationist",
  environs: "surroundings",
  envision: "imagine",
  eon: "long time",
  epic: "heroic",
  epidemic: "plague",
  epidermis: "skin",
  epistemology: "theory of knowledge",
  epitome: "embodiment",
  epitomize: "be typical of",
  epoch: "era",
  equestrian: "horse rider",
  equilibrium: "balance",
  equine: "horse-like",
  equivalence: "sameness",
  era: "epoch",
  eradicate: "destroy",
  erect: "build",
  erode: "deteriorate",
  erosion: "wearing away",
  erratic: "irregular",
  erroneous: "mistaken",
  escalate: "intensify",
  escape: "get away",
  escort: "accompany",
  esophagus: "gullet",
  especially: "particularly",
  essential: "necessary",
  esteem: "great regard",
  estrange: "alienate",
  eternal: "perpetual",
  ethical: "moral",
  ethics: "moral philosophy",
  etiquette: "manners",
  eulogize: "praise",
  euphonious: "sweet-sounding",
  euphoria: "bliss",
  evade: "dodge",
  evaluate: "assess",
  evanescence: "gradual vanishment",
  evangelize: "preach",
  evasion: "avoidance",
  eventuality: "possibility",
  everlasting: "eternal",
  evermore: "forever",
  evil: "wicked",
  exacerbate: "make worse",
  exact: "precise",
  exactly: "precisely",
  exaggerate: "overstate",
  exaggeration: "overstatement",
  exalt: "glorify",
  exasperate: "greatly irritate",
  excavate: "dig up",
  excavation: "digging",
  exceedingly: "extremely",
  excel: "surpass",
  excellent: "superior",
  excerpt: "selection",
  excess: "surplus",
  excessive: "overly much",
  exclamation: "outcry",
  exclude: "omit",
  exclusive: "unshared",
  excrescence: "outgrowth",
  excruciating: "agonizing",
  excursion: "outing",
  exemplar: "model",
  exemplify: "illustrate",
  exemption: "immunity",
  exert: "wield",
  exertion: "effort",
  exhaustive: "thorough",
  exhibit: "display",
  exhibition: "display",
  exhilarate: "thrill",
  exhilaration: "euphoria",
  exhume: "dig up from a grave",
  existence: "being",
  exodus: "departure",
  exonerate: "acquit",
  exorbitance: "excess",
  exorbitant: "excessive",
  exothermic: "heat-releasing",
  expanse: "area",
  expect: "anticipate",
  expedite: "hasten",
  expend: "use up",
  expendable: "non-essential",
  expense: "cost",
  expertise: "know-how",
  expire: "die",
  explicate: "explain fully",
  explode: "burst",
  explosion: "detonation",
  expose: "reveal",
  expressly: "explicitly",
  expulsion: "ejection",
  expunge: "erase",
  extensive: "wide",
  exterior: "outside",
  external: "outside",
  externally: "outwardly",
  extinct: "died out",
  extra: "additional",
  extract: "pull out",
  extraordinary: "remarkable",
  extrasensory: "paranormal",
  extraterrestrial: "alien",
  extravagant: "lavish",
  extremist: "radical",
  extricate: "disentangle",
  extroverted: "outgoing",
  extrude: "thrust out",
  exuberance: "enthusiasm",
  exuberant: "overjoyed",
  exude: "ooze out",
  exult: "rejoice",
  exultant: "joyous",
  exultation: "jubilance",
  fabric: "cloth",
  fabricate: "construct",
  fabrication: "made-up story",
  fabulist: "storyteller",
  fabulous: "incredible",
  facade: "front of a building",
  faceless: "anonymous",
  facet: "aspect",
  facsimile: "copy",
  fact: "certainty",
  faction: "clique",
  fad: "craze",
  faint: "weak",
  faith: "trust",
  faithful: "loyal",
  faithless: "disloyal",
  fallacy: "misconception",
  fallible: "liable to make errors",
  falsetto: "high male voice",
  falter: "stumble",
  familiarity: "intimacy",
  familiarize: "accustom",
  famine: "starvation",
  famish: "starve",
  famished: "starving",
  fanatic: "extremist",
  fanaticism: "zealotry",
  fancier: "enthusiast",
  fanciful: "whimsical",
  fanfare: "hullabaloo",
  fantasize: "imagine",
  farce: "ridiculous comedy",
  fascinating: "very interesting",
  fascism: "dictatorship",
  fashionable: "stylish",
  fasten: "attach",
  fastidious: "very particular",
  fatal: "deadly",
  fatality: "death",
  fate: "destiny",
  fated: "destined",
  fateful: "momentous",
  fatigue: "weariness",
  fatuousness: "absurdity",
  faucet: "spigot",
  faulty: "imperfect",
  faux: "imitation",
  favor: "kindness",
  faze: "daunt",
  fear: "dread",
  fearless: "brave",
  feasible: "practicable",
  feast: "banquet",
  feat: "deed",
  feeble: "weak",
  feebleminded: "foolish",
  feigned: "pretended",
  feline: "catlike",
  felony: "serious crime",
  feminine: "womanly",
  femur: "thighbone",
  fend: "ward off",
  ferocious: "fierce",
  ferocity: "fierceness",
  fervency: "ardor",
  fervent: "zealous",
  festive: "celebratory",
  fetch: "get",
  fetching: "attractive",
  fib: "falsehood",
  fibrous: "sinewy",
  fibula: "shinbone",
  fickle: "inconstant",
  fictitious: "imaginary",
  fidelity: "loyalty",
  fiendish: "diabolical",
  fierce: "ferocious",
  fiercely: "ferociously",
  figment: "fabrication",
  figurative: "metaphorical",
  filament: "strand",
  filthy: "dirty",
  finale: "conclusion",
  financial: "monetary",
  financing: "funding",
  finery: "clothes",
  finicky: "particular",
  finis: "end",
  finite: "limited",
  firebug: "pyromaniac",
  firm: "solid",
  fishy: "suspicious",
  fission: "splitting",
  fissure: "crack",
  fisticuffs: "boxing",
  fitting: "suitable",
  fixation: "obsession",
  fjord: "inlet",
  flaccid: "limp",
  flagrant: "blatant",
  flail: "thrash",
  flair: "style",
  flame: "blaze",
  flapjack: "pancake",
  flashy: "gaudy",
  flatter: "compliment excessively",
  flaunt: "show off",
  flaw: "defect",
  flay: "strip the skin off",
  fledgling: "immature",
  flee: "run away",
  fleet: "swift",
  flex: "bend",
  flimflam: "trick",
  flimsy: "weak",
  flippant: "smart-alecky",
  floaty: "buoyant",
  flock: "group",
  flog: "whip",
  flora: "plants",
  florescence: "blossoming",
  flotilla: "small fleet",
  flotsam: "wreckage",
  flounce: "ruffle",
  flounder: "struggle",
  flourish: "thrive",
  flowery: "ornate",
  flub: "botch",
  fluctuate: "waver",
  fluke: "good fortune",
  flunk: "fail",
  fluster: "upset",
  flux: "change",
  foamy: "frothy",
  foe: "enemy",
  foggy: "misty",
  folksy: "homespun",
  folly: "foolishness",
  fond: "affectionate",
  fondle: "caress",
  fondly: "affectionately",
  fondness: "affection",
  font: "typeface",
  foolish: "unwise",
  footfall: "step",
  footnote: "annotation",
  forbearance: "patience",
  forbid: "prohibit",
  forbidden: "prohibited",
  foreboding: "premonition",
  forecast: "predict",
  forefather: "ancestor",
  foreigner: "outsider",
  foreman: "boss",
  foreordain: "predetermine",
  forepart: "front",
  foresee: "predict",
  foreshadow: "indicate in advance",
  foresight: "knowledge of the future",
  forestall: "prevent",
  foretell: "predict",
  forever: "eternally",
  forfeit: "lose",
  forger: "counterfeiter",
  forgery: "counterfeit",
  forgive: "pardon",
  forgo: "give up",
  forked: "branched",
  forlorn: "miserable",
  format: "arrangement",
  former: "previous",
  formidable: "impressive",
  formulate: "devise",
  forsake: "abandon",
  forte: "strength",
  fortify: "strengthen",
  fortitude: "courage",
  fortunately: "luckily",
  fortune: "luck",
  foster: "nurture",
  fountainhead: "wellspring",
  foyer: "lobby",
  fracture: "break",
  fragility: "brittleness",
  fragmentary: "disconnected",
  fragrance: "scent",
  fragrant: "sweet-smelling",
  frail: "weak",
  frailty: "weakness",
  frank: "candid",
  frankly: "candidly",
  frankness: "candor",
  frantic: "frenzied",
  fraternal: "brotherly",
  fraud: "deceit",
  fraudulence: "deceitfulness",
  fraudulent: "deceitful",
  freaky: "bizarre",
  freelance: "independent",
  freight: "cargo",
  frenzied: "frantic",
  frequently: "often",
  freshly: "newly",
  fretful: "irritable",
  frigid: "cold",
  frill: "ruffle",
  fringe: "border",
  frisky: "playful",
  frolic: "romp",
  frolicsome: "playful",
  frosty: "cold",
  froth: "foam",
  fructose: "fruit sugar",
  frugal: "economical",
  frugality: "thrift",
  fruitful: "productive",
  fugitive: "runaway",
  fulfill: "satisfy",
  fumble: "mishandle",
  fun: "merriment",
  functional: "working",
  fundamental: "essential",
  fungous: "spongy",
  funny: "comical",
  furious: "angry",
  furl: "wrap",
  furtherance: "advancement",
  fury: "rage",
  futile: "useless",
  futility: "uselessness",
  gab: "chatter",
  gaff: "hooked fishing spear",
  gaggle: "flock",
  gaiety: "merriment",
  gaily: "merrily",
  gainful: "profitable",
  gala: "festivity",
  gallant: "brave",
  gallantry: "bravery",
  galleon: "sailing ship",
  galore: "in abundance",
  gander: "male goose",
  gangling: "lanky",
  gangster: "racketeer",
  gape: "yawn",
  garbanzo: "chickpea",
  gargantuan: "huge",
  garland: "wreath",
  garner: "collect",
  garnish: "decorate",
  gash: "cut",
  gasoline: "petrol",
  gasp: "pant",
  gather: "collect",
  gathering: "assembly",
  gaudy: "showy",
  gavel: "mallet",
  gawky: "clumsy",
  gaze: "stare",
  gazebo: "pavilion",
  gazette: "newspaper",
  gazpacho: "soup",
  gelatinous: "jelly-like",
  gem: "jewel",
  generally: "commonly",
  genesis: "beginning",
  genetic: "hereditary",
  genial: "friendly",
  genre: "category",
  genteel: "polite",
  gentility: "refinement",
  gentry: "aristocracy",
  genuine: "authentic",
  germicide: "disinfectant",
  germinate: "sprout",
  gestation: "pregnancy",
  ghastly: "horrible",
  ghoul: "fiend",
  gibber: "chatter",
  gibberish: "babble",
  gibe: "scoff",
  gifted: "talented",
  gigantic: "tremendous",
  gimp: "hobble",
  gingerly: "cautiously",
  girder: "beam",
  girth: "circumference",
  gisarme: "halberd",
  gist: "essence",
  glacial: "icy",
  glaciate: "freeze",
  glad: "happy",
  glade: "clearing",
  glance: "quick look",
  glaring: "flagrant",
  glazier: "glass-cutter",
  gleam: "glitter",
  glee: "merriment",
  gleeful: "merry",
  glib: "smooth-tongued",
  glimmer: "faint light",
  glisten: "sparkle",
  glitch: "problem",
  glitter: "sparkle",
  glitz: "flashiness",
  global: "worldwide",
  globular: "spherical",
  gloomy: "melancholy",
  glorify: "exalt",
  glorious: "splendid",
  gloss: "polish",
  glum: "sullen",
  glutton: "over-eater",
  gluttonous: "voracious",
  gluttony: "excess in eating",
  gnarled: "knotty",
  gnash: "grind",
  gnaw: "chew",
  gnome: "dwarf",
  gob: "lump",
  goodwill: "kindliness",
  goody: "delicacy",
  goof: "mistake",
  gore: "bloodiness",
  gory: "bloody",
  gourmet: "food connoisseur",
  gown: "robe",
  gracious: "courteous",
  grand: "magnificent",
  grandeur: "magnificence",
  grandiloquence: "pompous speech",
  grandiloquent: "pompous",
  grandiose: "impressive",
  graphic: "vivid",
  grapple: "wrestle",
  grasping: "greedy",
  gratify: "satisfy",
  gratifying: "pleasing",
  gratis: "free",
  grease: "lubricate",
  greasy: "oily",
  grief: "sorrow",
  grievance: "complaint",
  grieve: "mourn",
  grim: "stern",
  grime: "dirt",
  grimy: "dirty",
  grisly: "gruesome",
  gristle: "cartilage",
  groan: "moan",
  groggy: "unsteady",
  groove: "thin channel",
  grope: "fumble",
  grotesque: "bizarre",
  grouch: "ill-tempered person",
  groundbreaking: "innovative",
  groundless: "unfounded",
  groundwork: "foundation",
  grub: "larva",
  grudge: "grievance",
  grudging: "reluctant",
  gruel: "porridge",
  gruff: "brusque",
  grump: "complainer",
  grunge: "filth",
  guardian: "protector",
  guffaw: "hearty laugh",
  guile: "cunning",
  guise: "appearance",
  gulch: "ravine",
  gullible: "easily deceived",
  gully: "ravine",
  gulp: "swig",
  gummy: "sticky",
  gush: "spurt",
  gushy: "overly sentimental",
  gusto: "zest",
  gutless: "cowardly",
  gutsy: "courageous",
  guzzle: "gulp",
  gyrate: "revolve",
  gyration: "circular motion",
  habit: "personal custom",
  habitable: "fit to live in",
  habitat: "natural environment",
  habitation: "home",
  habitual: "customary",
  habituate: "accustom",
  hacienda: "plantation",
  haggard: "gaunt",
  halfhearted: "lacking enthusiasm",
  halitosis: "bad breath",
  hallowed: "sacred",
  hallucination: "delusion",
  halo: "ring of light",
  halt: "stop",
  hamlet: "small village",
  hammerlock: "wrestling hold",
  handicap: "disability",
  handicapper: "odds-maker",
  handily: "conveniently",
  handrail: "banister",
  handsome: "fine-looking",
  hangdog: "shamefaced",
  hanker: "yearn",
  happening: "occurrence",
  harass: "annoy",
  hardening: "solidification",
  hardly: "barely",
  harebrained: "foolish",
  harmonic: "musically pleasing",
  harrowing: "traumatic",
  harsh: "severe",
  harvest: "crop",
  haughty: "arrogant",
  haven: "harbor",
  havoc: "devastation",
  hawker: "peddler",
  haywire: "crazy",
  hazard: "endangerment",
  hazardous: "dangerous",
  haze: "fog",
  hazy: "misty",
  headquarters: "main office",
  headsman: "executioner",
  headstrong: "willful",
  heal: "cure",
  hearsay: "rumor",
  heartache: "grief",
  hearten: "encourage",
  heartfelt: "earnest",
  heartless: "cruel",
  heartrending: "sorrowful",
  heathen: "pagan",
  heckle: "taunt",
  hedonist: "pleasure-seeker",
  heed: "mind",
  heedful: "attentive",
  heedless: "inattentive",
  hefty: "substantial",
  heifer: "cow",
  heinous: "atrocious",
  heir: "inheritor",
  heist: "robbery",
  heliocentric: "sun-centered",
  helix: "spiral",
  help: "aid",
  helper: "assistant",
  hemorrhage: "bleed",
  henchman: "crony",
  herald: "proclaim",
  herbage: "vegetation",
  herbivorous: "plant-eating",
  heresy: "unorthodoxy",
  hermaphrodite: "androgyne",
  hermit: "loner",
  heroism: "valor",
  hesitate: "waver",
  hiatus: "gap",
  hibernal: "wintry",
  hidden: "concealed",
  hide: "conceal",
  hideous: "ugly",
  hijack: "commandeer",
  hilarious: "extremely funny",
  hilarity: "mirth",
  hillock: "mound",
  hinder: "obstruct",
  hindmost: "rear",
  hindrance: "obstacle",
  hinterland: "boondocks",
  hobo: "tramp",
  hodgepodge: "jumble",
  hogwash: "nonsense",
  hoist: "lift",
  holler: "shout",
  holster: "pistol case",
  holy: "sacred",
  homage: "respect",
  homestead: "house",
  homicidal: "murderous",
  homicide: "murder",
  hominid: "man-like",
  homogeneous: "similar",
  hone: "sharpen",
  hoodlum: "thug",
  hoodwink: "deceive",
  hooey: "nonsense",
  hooky: "truancy",
  horologist: "watchmaker",
  horrible: "dreadful",
  horror: "dread",
  horseplay: "rowdiness",
  hosiery: "stockings",
  hospitable: "welcoming",
  hostel: "inn",
  hostile: "unfriendly",
  hostility: "unfriendliness",
  hotel: "inn",
  hound: "dog",
  hovel: "run-down hut",
  howl: "wail",
  hubbub: "uproar",
  hue: "color",
  huffy: "touchy",
  hug: "embrace",
  hulk: "loom",
  hulking: "massive",
  hullabaloo: "uproar",
  humane: "compassionate",
  humble: "modest",
  humdrum: "monotonous",
  humid: "steamy",
  humility: "meekness",
  humorous: "comical",
  hurl: "throw",
  hurrah: "cheer",
  hurtful: "injurious",
  husk: "shell",
  hut: "shack",
  hutch: "shack",
  hybrid: "composite",
  hybridize: "crossbreed",
  hydrophobia: "fear of water",
  hydrophyte: "aquatic plant",
  hygienic: "sanitary",
  hype: "publicity",
  hyperbole: "exaggeration",
  hyperbolic: "exaggerated",
  hyperbolize: "exaggerate",
  hypercritical: "faultfinding",
  hypermetropia: "farsightedness",
  hyperpyrexia: "high fever",
  hypertension: "high blood pressure",
  hypocrisy: "phoniness",
  hypothermia: "low body temperature",
  hypothesis: "theory",
  hypothesize: "speculate",
  hypothetical: "conjectural",
  icing: "frosting",
  icky: "distasteful",
  icon: "image",
  icy: "frigid",
  idealist: "dreamer",
  idiom: "expression",
  idiosyncrasy: "eccentricity",
  idiotic: "foolish",
  idolize: "worship",
  ignoble: "dishonorable",
  ignoramus: "dunce",
  ignorant: "uninformed",
  ill: "sick",
  illegal: "unlawful",
  illicit: "unlawful",
  illumination: "light",
  illusionist: "magician",
  illusive: "deceptive",
  illusory: "deceptive",
  imbecile: "idiot",
  imbecility: "foolishness",
  imitation: "copy",
  immaculate: "spotless",
  immaterial: "unimportant",
  immature: "undeveloped",
  immeasurable: "vast",
  immediately: "right away",
  immense: "vast",
  immerse: "plunge",
  imminent: "impending",
  immobile: "stationary",
  immoderate: "excessive",
  immoral: "unethical",
  immortal: "living forever",
  immune: "exempt",
  immunity: "exemption",
  immutable: "unchanging",
  imp: "pixie",
  impair: "worsen",
  impale: "spike",
  impartial: "unbiased",
  impassioned: "emotionally excited",
  impassive: "unmoved",
  impatient: "restless",
  impeach: "accuse of a crime",
  impeccable: "faultless",
  impedance: "resistance",
  impede: "hinder",
  impediment: "obstruction",
  imperative: "obligatory",
  imperfection: "blemish",
  imperil: "endanger",
  impertinence: "rudeness",
  impetuosity: "rashness",
  impiety: "irreverence",
  impish: "mischievous",
  implant: "instill",
  implausible: "hard to believe",
  implement: "tool",
  implode: "burst inward",
  implosion: "inward bursting",
  imply: "suggest",
  impolite: "rude",
  imposition: "burden",
  impostor: "pretender",
  impotence: "powerlessness",
  impotent: "weak",
  impound: "seize",
  impoverish: "make poor",
  impoverished: "poor",
  impregnable: "unconquerable",
  impregnate: "fertilize",
  impressionable: "easily influenced",
  impressionism: "painting style",
  improbable: "unlikely",
  impromptu: "without preparation",
  improve: "make better",
  improvise: "invent",
  imprudent: "unwise",
  impudence: "rudeness",
  impulse: "urge",
  impunity: "freedom from punishment",
  impure: "tainted",
  impurity: "contamination",
  inaccuracy: "error",
  inaction: "idleness",
  inadequacy: "deficiency",
  inadequate: "deficient",
  inadvertent: "accidental",
  inadvertently: "unintentionally",
  inanimate: "lifeless",
  inapt: "unsuitable",
  inauspicious: "unfavorable",
  inborn: "innate",
  incandescent: "glowing",
  incapacitate: "disable",
  incarnation: "embodiment",
  incendiary: "inflammatory",
  inception: "beginning",
  incessant: "unceasing",
  incident: "event",
  incidental: "minor",
  incidentally: "by the way",
  incinerate: "burn",
  incision: "cut",
  incite: "provoke",
  incivility: "rudeness",
  inclination: "tendency",
  incline: "slope",
  incogitant: "thoughtless",
  incognito: "disguised",
  incomparable: "matchless",
  inconceivable: "unimaginable",
  inconsequential: "unimportant",
  inconsiderate: "thoughtless",
  inconstancy: "fickleness",
  incorrigible: "unreformable",
  incredulity: "disbelief",
  incredulous: "unbelieving",
  increment: "increase",
  incubate: "hatch",
  indebted: "beholden",
  indecisive: "wavering",
  indefatigability: "tirelessness",
  indefatigable: "untiring",
  indefensible: "unjustifiable",
  indefinite: "vague",
  indelicacy: "rudeness",
  indict: "accuse",
  indictment: "accusation",
  indifferent: "not caring",
  indigene: "native",
  indigenous: "native",
  indignant: "irate",
  indispensable: "essential",
  indisposed: "ailing",
  indisposition: "reluctance",
  indistinguishable: "identical",
  indolence: "laziness",
  indomitable: "unconquerable",
  induce: "cause",
  inducement: "incentive",
  indulge: "gratify",
  industrious: "hard-working",
  inebriate: "intoxicate",
  inebriety: "drunkenness",
  inefficient: "wasteful",
  inelastic: "rigid",
  inept: "clumsy",
  inequity: "unfairness",
  inert: "inactive",
  inexpensive: "cheap",
  infallible: "unerring",
  infamous: "notorious",
  infancy: "babyhood",
  infant: "baby",
  infantile: "childish",
  infantry: "foot soldiers",
  infantryman: "foot soldier",
  infatuated: "smitten",
  infer: "deduce",
  infernal: "fiendish",
  inferno: "raging fire",
  infertile: "barren",
  infest: "overrun",
  infidel: "unbeliever",
  infidelity: "unfaithfulness",
  infinite: "endless",
  infirm: "weak",
  infirmary: "hospital",
  inflammable: "combustible",
  inflexible: "rigid",
  inflict: "impose",
  informal: "casual",
  infraction: "violation",
  infrequent: "uncommon",
  infringe: "encroach",
  infringement: "violation",
  infuriate: "enrage",
  ingenious: "clever",
  ingenuity: "cleverness",
  ingest: "consume",
  ingrained: "deep-seated",
  ingrate: "thankless person",
  ingredient: "component",
  inhabitant: "resident",
  inhale: "breathe in",
  inharmonic: "discordant",
  inhibit: "restrain",
  inhibition: "restraint",
  inhuman: "savage",
  inhumanity: "cruelty",
  initial: "first",
  initiate: "begin",
  injurious: "harmful",
  injustice: "unfairness",
  inkling: "hint",
  inn: "hotel",
  innards: "entrails",
  innate: "inborn",
  innumerable: "countless",
  inoculate: "immunize",
  inopportune: "untimely",
  inquire: "ask",
  inquiry: "question",
  inquisitive: "curious",
  insane: "crazy",
  insanity: "madness",
  inscribe: "write",
  insight: "perception",
  insightful: "perceptive",
  insignia: "emblem",
  insignificant: "trivial",
  insinuate: "imply",
  insobriety: "drunkenness",
  insolence: "haughtiness",
  insolent: "disrespectful",
  insomnia: "sleeplessness",
  inspect: "examine",
  instance: "example",
  instancy: "urgency",
  instant: "immediate",
  instigate: "provoke",
  instigator: "provoker",
  instruct: "teach",
  instructor: "teacher",
  instrumentalist: "musician",
  insubordinate: "disobedient",
  insubordination: "disobedience",
  insurgence: "uprising",
  insurgent: "rebellious",
  integral: "essential",
  integrate: "unite",
  integrity: "honorableness",
  intelligent: "smart",
  intelligible: "understandable",
  intensify: "heighten",
  intensive: "concentrated",
  intent: "purpose",
  intention: "purpose",
  intentional: "deliberate",
  interloper: "intruder",
  intermediary: "go-between",
  intermediate: "in between",
  interpreter: "translator",
  interrogate: "question",
  interrogator: "questioner",
  intersect: "cross",
  intersection: "crossing",
  intimacy: "closeness",
  intolerable: "unbearable",
  intoxicant: "alcohol",
  intoxicated: "drunk",
  intoxication: "drunkenness",
  intricacy: "elaborateness",
  introduce: "present",
  introductory: "preliminary",
  introspection: "self-contemplation",
  introspective: "self-examining",
  intrude: "trespass",
  intrusion: "encroachment",
  intuition: "hunch",
  intuitive: "instinctual",
  invalidate: "nullify",
  invaluable: "priceless",
  invariable: "constant",
  inventory: "stock",
  invert: "reverse",
  invertebrate: "spineless",
  invigorate: "energize",
  invincible: "unconquerable",
  invocation: "prayer",
  invoice: "bill",
  iota: "tiny bit",
  irate: "angry",
  ire: "anger",
  irk: "annoy",
  irksome: "annoying",
  irrational: "illogical",
  irrelevant: "unrelated",
  irresistible: "overpowering",
  irresolute: "indecisive",
  irresponsible: "unreliable",
  irrevocable: "unalterable",
  irrigate: "water",
  isle: "small island",
  islet: "little island",
  isolated: "alone",
  itemize: "list",
  itinerary: "travel route",
  jab: "thrust",
  jabber: "chatter",
  jacket: "coat",
  jaded: "wearied",
  jailbird: "convict",
  jargon: "lingo",
  jaunty: "lively",
  jealous: "envious",
  jealousy: "envy",
  jell: "congeal",
  jenny: "female donkey",
  jeopardize: "imperil",
  jeopardy: "danger",
  jest: "joke",
  jester: "fool",
  jetty: "breakwater",
  jewel: "gem",
  jiffy: "instant",
  jiggle: "shake",
  jimmies: "chocolate sprinkles",
  jimmy: "pry",
  jittery: "nervous",
  joey: "baby kangaroo",
  join: "connect",
  jointly: "together",
  jolly: "in good spirits",
  josh: "tease jokingly",
  jounce: "jolt",
  journal: "diary",
  jovial: "merry",
  jowl: "jaw",
  jubilant: "joyful",
  jubilate: "rejoice",
  jubilation: "rejoicing",
  judicious: "wise",
  jugular: "throat",
  jumble: "hodgepodge",
  jumbo: "huge",
  jumpy: "nervous",
  jurisprudence: "law",
  justify: "warrant",
  juvenile: "young",
  keen: "acute",
  keepsake: "memento",
  keg: "cask",
  kempt: "tidy",
  kettle: "pot",
  kid: "child",
  kidnap: "abduct",
  kiln: "oven",
  kin: "relatives",
  kindling: "tinder",
  kindred: "related",
  kingdom: "realm",
  kink: "twist",
  kinsman: "relative",
  kiosk: "stall",
  kitten: "young cat",
  knack: "talent",
  knickknack: "trinket",
  knightly: "chivalrous",
  knockoff: "imitation",
  knotty: "complex",
  kook: "eccentric",
  label: "tag",
  labor: "work",
  laborer: "workman",
  laborious: "toilsome",
  labyrinth: "maze",
  labyrinthine: "maze-like",
  lacerate: "tear",
  laceration: "wound",
  lack: "be without",
  lackluster: "dull",
  lacteal: "milky",
  lactic: "milk-related",
  lactose: "milk sugar",
  laddie: "young man",
  laden: "loaded",
  lag: "be behind",
  laggard: "straggler",
  lame: "crippled",
  lancet: "surgical knife",
  lanky: "gangling",
  lapse: "slip",
  larcenist: "burglar",
  larceny: "theft",
  lard: "fat",
  lash: "whip",
  lass: "girl",
  lassie: "young girl",
  lasting: "durable",
  latency: "inactivity",
  lattice: "grille",
  lavatory: "bathroom",
  lavish: "profuse",
  lax: "slack",
  laxity: "looseness",
  layman: "nonprofessional",
  layout: "arrangement",
  leaflet: "brochure",
  lease: "rent",
  least: "smallest",
  leatherback: "sea turtle",
  leave: "depart",
  leer: "ogle",
  leery: "wary",
  leftover: "remnant",
  legendary: "fabled",
  legible: "readable",
  legion: "multitude",
  legitimate: "lawful",
  leisure: "free time",
  lengthen: "extend",
  lenient: "merciful",
  lesion: "wound",
  lessen: "decrease",
  lethal: "deadly",
  lethargic: "sluggish",
  lethargy: "sluggishness",
  lewd: "obscene",
  liaison: "go-between",
  liberate: "free",
  libretto: "text of an opera",
  license: "permit",
  lighthouse: "beacon",
  liken: "compare",
  liking: "fondness",
  limber: "flexible",
  limitation: "restriction",
  limited: "restricted",
  lineage: "ancestry",
  linger: "stay around",
  lingo: "jargon",
  lingua: "tongue",
  link: "join",
  linkage: "connection",
  liquor: "alcohol",
  listless: "lethargic",
  literate: "able to read and write",
  liven: "invigorate",
  loafer: "idler",
  loathe: "detest",
  locality: "neighborhood",
  lockjaw: "tetanus",
  loco: "crazy",
  lodging: "shelter",
  lofty: "towering",
  logistics: "practical details",
  loiter: "linger",
  loll: "droop",
  lollygag: "dawdle",
  lone: "solitary",
  longing: "yearning",
  longitudinal: "running lengthwise",
  longshoreman: "dock worker",
  lop: "cut off",
  lopsided: "unbalanced",
  lordly: "grand",
  lore: "knowledge",
  lottery: "sweepstake",
  lousy: "contemptible",
  loyal: "faithful",
  lucid: "clear",
  lucidity: "clearness",
  lucrative: "profitable",
  ludicrous: "absurd",
  lug: "haul",
  luggage: "baggage",
  lull: "calm",
  lumber: "wooden boards",
  luminance: "brightness",
  luminesce: "glow",
  luminescence: "glow",
  lump: "chunk",
  lumpy: "chunky",
  lunacy: "insanity",
  lunatic: "madman",
  lunge: "thrust",
  lurch: "stagger",
  luscious: "delectable",
  lustrous: "shining",
  luxuriant: "lush",
  lyre: "harp",
  macaroni: "tubular pasta",
  mace: "medieval battle club",
  machismo: "manliness",
  macho: "manly",
  madcap: "wild",
  madman: "lunatic",
  madness: "insanity",
  maestro: "music master",
  magician: "sorcerer",
  magisterial: "authoritative",
  magistrate: "judge",
  magnify: "enlarge",
  magniloquence: "pompous speech",
  maiden: "young woman",
  maim: "mutilate",
  mainly: "chiefly",
  maintenance: "upkeep",
  maize: "corn",
  majestic: "stately",
  majesty: "grandeur",
  makeshift: "improvised",
  malady: "illness",
  malaise: "weary uneasiness",
  malcontent: "dissatisfied person",
  malevolence: "ill will",
  malevolent: "hateful",
  malice: "hatred",
  malicious: "hateful",
  mallard: "duck",
  mammoth: "gigantic",
  mandate: "command",
  mandible: "jaw",
  mangle: "mutilate",
  mangy: "shabby",
  mania: "insanity",
  maniac: "madman",
  maniacal: "insane",
  manifesto: "declaration",
  mannerly: "courteous",
  manual: "handbook",
  manufacturer: "maker",
  mar: "spoil",
  marauder: "plunderer",
  mare: "female horse",
  margarine: "oleo",
  marginal: "borderline",
  marine: "oceanic",
  mariner: "seaman",
  marionette: "puppet",
  maritime: "nautical",
  marksman: "sharpshooter",
  maroon: "strand",
  marsh: "swamp",
  martial: "warlike",
  marvel: "wonder",
  marvelous: "wonderful",
  masculine: "male",
  mash: "crush",
  masquerade: "disguise",
  mass: "bulk",
  massacre: "slaughter",
  massive: "huge",
  masterly: "skillful",
  mastermind: "genius",
  maternal: "motherly",
  maternity: "motherhood",
  matrimony: "marriage",
  matte: "non-glossy",
  mature: "full-grown",
  maul: "mangle",
  mausoleum: "tomb",
  maverick: "rebel",
  maximum: "greatest",
  maybe: "perhaps",
  mayhem: "havoc",
  maze: "labyrinth",
  meager: "scanty",
  meander: "ramble",
  meaningful: "significant",
  measly: "scanty",
  medal: "award",
  medallion: "large medal",
  meddle: "interfere",
  medial: "middle",
  median: "middle",
  mediocre: "ordinary",
  mediocrity: "averageness",
  meditate: "reflect",
  medley: "mixture",
  meek: "humble",
  megalopolis: "large city",
  melancholic: "depressed",
  melancholy: "gloomy",
  mellow: "relaxed",
  melodious: "musical",
  melodramatic: "overly theatrical",
  melody: "tune",
  melt: "dissolve",
  memento: "souvenir",
  memoir: "autobiography",
  memorandum: "note",
  memorialize: "commemorate",
  menace: "threat",
  mend: "repair",
  mentality: "mind-set",
  mentor: "wise counselor",
  mercantile: "commercial",
  merchandise: "products",
  merchantman: "freighter",
  merciful: "compassionate",
  merciless: "cruel",
  mercury: "quicksilver",
  merit: "virtue",
  meritorious: "praiseworthy",
  mesmerize: "spellbind",
  metamorphosis: "transformation",
  metaphor: "analogy",
  metaphysics: "philosophy",
  methodical: "systematic",
  meticulous: "precise",
  metropolis: "city",
  microbe: "germ",
  microcosm: "miniature world",
  middling: "mediocre",
  midst: "middle",
  migraine: "headache",
  migrant: "wandering",
  migratory: "roving",
  mild: "gentle",
  militant: "combative",
  millesimal: "thousandth",
  mimic: "imitate",
  mimicry: "imitation",
  mindful: "aware",
  mingle: "intermix",
  miniature: "small-scale",
  minnow: "small fish",
  minor: "less important",
  minstrel: "medieval singer",
  minuet: "dance",
  mirage: "illusion",
  misanthropy: "hatred of humankind",
  miscellaneous: "assorted",
  miscellany: "assortment",
  miscreant: "villain",
  miscue: "slip-up",
  misdeed: "wrongdoing",
  misdemeanor: "infraction",
  miserable: "wretched",
  misery: "unhappiness",
  misfortune: "bad luck",
  mishap: "accident",
  mishmash: "hodgepodge",
  mislay: "lose",
  misshapen: "deformed",
  missile: "projectile",
  mist: "fog",
  mistreat: "abuse",
  mitt: "glove",
  mobile: "movable",
  mobilize: "marshal",
  mock: "jeer",
  mockery: "ridicule",
  mode: "manner",
  moderate: "restrained",
  moderation: "restraint",
  modern: "up-to-date",
  modesty: "humility",
  modification: "change",
  modify: "change",
  mogul: "tycoon",
  moist: "damp",
  moisture: "wetness",
  mojo: "charm",
  moldy: "musty",
  mollusk: "shellfish",
  molten: "melted",
  momentary: "fleeting",
  monarch: "ruler",
  monetary: "financial",
  mongrel: "mutt",
  monochromatic: "single-colored",
  monocle: "eyeglass",
  monocracy: "dictatorship",
  monologue: "soliloquy",
  monotony: "sameness",
  monumental: "massive",
  moody: "glum",
  moony: "dreamy",
  mope: "pout",
  moral: "ethical",
  morale: "spirit",
  morality: "ethics",
  morsel: "small bite",
  mortgage: "property loan",
  mortician: "undertaker",
  mortify: "chagrin",
  motif: "theme",
  motto: "slogan",
  mourn: "grieve",
  mournful: "sad",
  mourning: "grief",
  mousy: "drab",
  muck: "gunk",
  muddle: "confuse",
  mug: "cup",
  muggy: "steamy",
  mull: "ponder",
  multifaceted: "many-sided",
  multifold: "numerous",
  multilateral: "many-sided",
  multitudinous: "many",
  mum: "silent",
  mumble: "mutter",
  munch: "chew",
  mundane: "ordinary",
  munition: "weaponry",
  murderous: "homicidal",
  muse: "ponder",
  mushy: "sentimental",
  muss: "disarrange",
  mussy: "disheveled",
  muster: "assemble",
  musty: "stale",
  mute: "silent",
  muted: "quiet",
  mutilate: "maim",
  mutinous: "insubordinate",
  mutiny: "rebellion",
  mutton: "sheep meat",
  mutual: "reciprocal",
  myalgia: "muscle pain",
  mycosis: "fungal disease",
  myocardium: "heart muscle",
  myopia: "nearsightedness",
  myopic: "nearsighted",
  mystical: "mysterious",
  mystify: "bewilder",
  mystique: "aura",
  myth: "legend",
  mythological: "legendary",
  nab: "apprehend",
  nape: "back of the neck",
  narcissistic: "self-obsessed",
  narrate: "tell",
  narration: "story",
  narrator: "storyteller",
  nasty: "filthy",
  natal: "birth-related",
  nativity: "birth",
  naturalize: "adapt",
  naughty: "misbehaving",
  nauseate: "disgust",
  nauseating: "repulsive",
  nauseous: "queasy",
  nautical: "maritime",
  navel: "bellybutton",
  neat: "tidy",
  neaten: "tidy",
  nebula: "galaxy",
  necropolis: "cemetery",
  needless: "unnecessary",
  needlework: "embroidery",
  needy: "poor",
  negate: "nullify",
  negation: "denial",
  neglect: "disregard",
  neglectful: "careless",
  negligence: "carelessness",
  negligent: "careless",
  neigh: "whinny",
  neonatal: "newborn",
  neophyte: "beginner",
  nervous: "uneasy",
  nestle: "cuddle",
  nestling: "young bird",
  nethermost: "lowest",
  neutralize: "counteract",
  newborn: "infant",
  newt: "salamander",
  nigh: "near",
  nightstick: "truncheon",
  nil: "nothing",
  nincompoop: "fool",
  nitpick: "quibble",
  nobility: "aristocracy",
  noctambulism: "sleepwalking",
  nocturnal: "active at night",
  noiseless: "silent",
  nomad: "wanderer",
  nomadic: "wandering",
  nominee: "candidate",
  nonchalance: "indifference",
  nonchalant: "unconcerned",
  nondescript: "ordinary",
  nonpartisan: "unbiased",
  nonsensical: "absurd",
  nook: "corner",
  norm: "average",
  normally: "usually",
  nosy: "prying",
  noteworthy: "remarkable",
  notify: "inform",
  notorious: "infamous",
  nourish: "feed",
  nourishing: "nutritious",
  nourishment: "sustenance",
  nova: "bright star",
  novelty: "newness",
  novice: "beginner",
  noxious: "harmful",
  nuclear: "atomic",
  nucleus: "core",
  nugget: "chunk",
  nuisance: "pest",
  nullification: "voiding",
  nullify: "invalidate",
  numerate: "count",
  numismatics: "coin collecting",
  numismatist: "coin collector",
  numskull: "dunce",
  nunnery: "convent",
  nursery: "baby's room",
  nutritious: "nourishing",
  nutty: "crazy",
  oaf: "lout",
  oarsman: "rower",
  obese: "overweight",
  objectivity: "impartiality",
  obligation: "duty",
  obligatory: "required",
  obliging: "helpful",
  obliterate: "destroy",
  oblivious: "unmindful",
  obscene: "indecent",
  obscure: "vague",
  observant: "attentive",
  observe: "watch",
  obsession: "fixation",
  obsolete: "outdated",
  obstacle: "impediment",
  obstinacy: "stubbornness",
  obstinate: "stubborn",
  obstruct: "impede",
  obstruction: "impediment",
  obtain: "get",
  obtrude: "thrust out",
  obtuse: "dull-witted",
  occupation: "job",
  occur: "happen",
  occurrence: "happening",
  octagon: "eight-sided figure",
  ocular: "visual",
  oculist: "eye doctor",
  oddball: "eccentric",
  odds: "probability",
  ode: "lyric poem",
  odious: "hateful",
  odor: "scent",
  odoriferous: "scented",
  odorous: "fragrant",
  odyssey: "adventurous journey",
  offbeat: "unconventional",
  offspring: "children",
  often: "frequently",
  olfactory: "related to smelling",
  oligarchy: "government by a few",
  ominous: "threatening",
  omit: "exclude",
  omnipotence: "unlimited power",
  omnipresent: "existing everywhere",
  omniscience: "infinite knowledge",
  omniscient: "all-knowing",
  ongoing: "continuing",
  onlooker: "spectator",
  onset: "beginning",
  onslaught: "attack",
  oodles: "lots",
  opacity: "non-transparency",
  opaque: "non-transparent",
  openhanded: "generous",
  operable: "functional",
  opportune: "timely",
  opportunity: "favorable occasion",
  oppose: "resist",
  opposition: "resistance",
  oppression: "tyranny",
  oppressive: "tyrannical",
  opt: "choose",
  optimal: "best",
  optimum: "best",
  option: "choice",
  oracle: "seer",
  oral: "spoken",
  oration: "speech",
  orator: "public speaker",
  orb: "globe",
  orbit: "revolve",
  orchestrate: "organize",
  ordain: "decree",
  orderly: "regular",
  ordinance: "regulation",
  ordnance: "artillery",
  organize: "arrange",
  origami: "paper-folding",
  origin: "source",
  originality: "inventiveness",
  originally: "at first",
  originate: "start",
  ornament: "decoration",
  ornithologist: "bird-watcher",
  orthodox: "conventional",
  oscillate: "swing",
  ossification: "hardening into bone",
  ostentation: "showiness",
  osteoid: "bonelike",
  ostracism: "banishment",
  oust: "remove",
  outbreak: "eruption",
  outburst: "eruption",
  outcome: "result",
  outdated: "obsolete",
  outer: "external",
  outfit: "equip",
  outhouse: "privy",
  outing: "excursion",
  outlying: "remote",
  output: "production",
  outrageous: "shocking",
  outrigger: "boat stabilizer",
  outset: "beginning",
  outsider: "foreigner",
  outskirt: "border",
  outskirts: "periphery",
  outspoken: "frank",
  outstanding: "superior",
  outstretch: "extend",
  outstrip: "surpass",
  outwardly: "externally",
  oval: "elliptical",
  ovate: "egg-shaped",
  ovation: "applause",
  overbearing: "dictatorial",
  overblown: "excessive",
  overcast: "cloudy",
  overcome: "surmount",
  overdue: "late",
  overflow: "flood",
  overhead: "above",
  overjoyed: "delighted",
  overlay: "cover",
  overlord: "master",
  overpower: "subdue",
  overriding: "predominant",
  overseas: "abroad",
  overshadow: "eclipse",
  overstate: "exaggerate",
  overstep: "exceed",
  overtake: "pass",
  overture: "prelude",
  overview: "summary",
  oviparous: "egg-laying",
  ovum: "egg cell",
  pacific: "peaceful",
  pacifist: "believer in non-violence",
  pacify: "appease",
  package: "parcel",
  pact: "treaty",
  pad: "cushion",
  paddock: "racehorse enclosure",
  paddy: "rice field",
  padre: "military chaplain",
  pagan: "heathen",
  pail: "bucket",
  painstaking: "diligent",
  pal: "friend",
  palatable: "savory",
  palate: "roof of the mouth",
  pallor: "paleness",
  palpitate: "throb",
  palsy: "paralysis",
  pamphlet: "booklet",
  pancake: "flapjack",
  pandemic: "widespread",
  pandemonium: "chaos",
  pang: "spasm",
  panic: "sudden fright",
  panorama: "wide view",
  pantry: "larder",
  paradox: "self-contradiction",
  paramount: "supreme",
  paranoid: "overly suspicious",
  paranormal: "extrasensory",
  paraphernalia: "assorted gear",
  paraphrase: "restate",
  parasol: "sunshade",
  parcel: "package",
  parch: "dry",
  parched: "dry",
  parchment: "sheepskin",
  pardon: "forgive",
  parole: "conditional release",
  parson: "clergyman",
  partake: "share",
  partiality: "bias",
  participation: "involvement",
  particularize: "specify",
  partition: "division",
  partnership: "alliance",
  passable: "adequate",
  passenger: "rider",
  passion: "emotion",
  passive: "inactive",
  pastoral: "idyllic",
  pasty: "pallid",
  patch: "mend",
  patchwork: "hodgepodge",
  paternal: "fatherly",
  paternity: "fatherhood",
  pathetic: "sad",
  patio: "terrace",
  patrician: "aristocratic",
  patron: "supporter",
  patronage: "support",
  patsy: "pushover",
  paunch: "belly",
  pauper: "beggar",
  pauperize: "impoverish",
  peachy: "dandy",
  peculiarity: "quirk",
  peddle: "sell",
  peddler: "hawker",
  pedestal: "base",
  pedestrian: "walker",
  pediatrician: "children's doctor",
  pedigree: "lineage",
  peep: "chirp",
  peer: "equal",
  penance: "atonement",
  pendulous: "hanging",
  penitentiary: "jail",
  pennant: "small flag",
  pensive: "thoughtful",
  pep: "liveliness",
  peppy: "lively",
  perceive: "observe",
  perception: "discernment",
  perch: "roost",
  percolate: "filter",
  perfectly: "completely",
  perforate: "pierce",
  perform: "accomplish",
  peril: "danger",
  perilous: "dangerous",
  perimeter: "border",
  periodical: "magazine",
  perish: "die",
  perishable: "easily spoiled",
  perky: "lively",
  permanence: "durability",
  permanent: "lasting",
  permeate: "spread through",
  permissible: "allowable",
  permission: "authorization",
  perpendicular: "vertical",
  perpetrator: "culprit",
  perpetual: "everlasting",
  perplex: "bewilder",
  perplexed: "puzzled",
  persecute: "oppress",
  persecution: "oppression",
  perseverance: "steadfastness",
  persistence: "doggedness",
  persnickety: "fussy",
  persona: "character",
  personable: "attractive",
  personify: "embody",
  perspiration: "sweat",
  perspire: "sweat",
  pertain: "relate",
  pertinence: "applicability",
  pertinent: "relevant",
  perturb: "agitate",
  perturbation: "disturbance",
  pervade: "spread",
  perverted: "depraved",
  pesky: "bothersome",
  pessimist: "naysayer",
  pessimistic: "gloomy",
  pester: "harass",
  petite: "small",
  petrol: "gasoline",
  petticoat: "underskirt",
  phantasm: "apparition",
  phantom: "apparition",
  pharmacist: "druggist",
  phenomenal: "extraordinary",
  philanderer: "womanizer",
  philatelist: "stamp collector",
  philistine: "uncultured person",
  phlebotomy: "bloodletting",
  phobia: "fear",
  phonate: "vocalize",
  phony: "fake",
  physician: "doctor",
  pianissimo: "very softly",
  piccolo: "flute",
  pickpocket: "thief",
  pier: "wharf",
  pierce: "penetrate",
  piety: "devoutness",
  pigheaded: "stubborn",
  pigment: "coloring",
  pilgrim: "wayfarer",
  pilgrimage: "journey",
  pillar: "column",
  pinnacle: "peak",
  pious: "devout",
  pirate: "buccaneer",
  pirouette: "whirl",
  pistol: "handgun",
  pitiful: "wretched",
  pittance: "small amount",
  pivot: "swivel",
  pivotal: "crucial",
  pixie: "sprite",
  placement: "position",
  placid: "calm",
  plaid: "checkered",
  plank: "board",
  plasterboard: "drywall",
  plateau: "elevated flatland",
  plausibility: "believableness",
  playful: "frolicsome",
  plaything: "toy",
  playwright: "dramatist",
  plea: "appeal",
  pleasant: "agreeable",
  pleasurable: "gratifying",
  pleat: "fold",
  plenitude: "abundance",
  plenteous: "abundant",
  plentiful: "abundant",
  plethora: "excess",
  pliable: "flexible",
  pliant: "flexible",
  plight: "condition",
  plod: "trudge",
  plot: "scheme",
  plotter: "conspirator",
  ploy: "stratagem",
  plug: "stopper",
  plumage: "feathers",
  plump: "chubby",
  plush: "luxurious",
  plushy: "lavish",
  pocketbook: "handbag",
  podiatrist: "foot doctor",
  podium: "stand",
  poise: "equilibrium",
  poisonous: "venomous",
  pollute: "contaminate",
  pomp: "splendor",
  pompous: "self-important",
  ponder: "contemplate",
  pontificate: "speak pompously",
  populace: "people",
  populate: "inhabit",
  populous: "crowded",
  porous: "permeable",
  portfolio: "collected drawings",
  portly: "stout",
  portrait: "picture",
  portray: "depict",
  portrayal: "depiction",
  posh: "elegant",
  positive: "certain",
  possibly: "perhaps",
  postage: "stamps",
  posterior: "rear",
  posthumous: "after death",
  postpone: "delay",
  postprandial: "after dinner",
  pot: "pan",
  potency: "power",
  potent: "powerful",
  pouch: "small sack",
  powerful: "mighty",
  pragmatic: "practical",
  pragmatist: "practical person",
  prankish: "mischievous",
  prattle: "babble",
  prawn: "shrimp",
  preamble: "introduction",
  precarious: "unstable",
  precedence: "priority",
  precinct: "district",
  precious: "treasured",
  precisely: "exactly",
  preconception: "bias",
  precondition: "requirement",
  precursor: "forerunner",
  predestinate: "foreordain",
  predestination: "fate",
  predict: "foretell",
  prediction: "prophecy",
  predisposition: "tendency",
  predominance: "superiority",
  preeminent: "outstanding",
  preen: "primp",
  prejudice: "bias",
  prejudiced: "biased",
  prejudicial: "detrimental",
  preliminary: "introductory",
  premeditation: "forethought",
  premier: "first",
  premise: "assumption",
  premonition: "forewarning",
  prenatal: "before birth",
  preoccupied: "distracted",
  preoccupy: "engross",
  preparation: "readiness",
  preposterous: "absurd",
  prerequisite: "requirement",
  prescience: "foresight",
  presentation: "display",
  pressing: "urgent",
  prestigious: "respected",
  pretend: "feign",
  pretext: "guise",
  prevalence: "frequency",
  prevalent: "widespread",
  preview: "advance showing",
  previous: "former",
  prey: "victim",
  priceless: "invaluable",
  pricey: "expensive",
  prickle: "thorn",
  prickly: "thorny",
  prim: "prudish",
  primal: "original",
  primarily: "mainly",
  primary: "main",
  primo: "first",
  principal: "main",
  prisoner: "captive",
  prissy: "prim",
  pristine: "unspoiled",
  pro: "in favor of",
  probability: "likelihood",
  probable: "likely",
  probably: "likely",
  probe: "examine",
  proceed: "continue",
  proclamation: "announcement",
  procrastinate: "postpone",
  procrastination: "delay",
  procreate: "reproduce",
  proctor: "supervisor",
  procure: "obtain",
  prod: "goad",
  profane: "unholy",
  profanity: "cursing",
  profess: "affirm",
  profession: "occupation",
  proficiency: "skill",
  profile: "outline",
  profit: "gain",
  profound: "deep",
  profuse: "abundant",
  profusion: "abundance",
  progenitor: "ancestor",
  prognostic: "foretelling",
  prognosticate: "predict",
  progress: "advancement",
  progressive: "advancing",
  prohibit: "forbid",
  prohibition: "ban",
  projectile: "missile",
  prologue: "introduction",
  promenade: "stroll",
  prominent: "conspicuous",
  promise: "pledge",
  prompt: "punctual",
  pronouncement: "declaration",
  pronto: "quickly",
  prop: "support",
  proper: "appropriate",
  prophecy: "foretelling",
  prophesy: "predict",
  prophet: "seer",
  prophetic: "predictive",
  proponent: "advocate",
  proportion: "ratio",
  proposal: "suggestion",
  propose: "suggest",
  proprietor: "owner",
  prose: "writing",
  prosper: "thrive",
  prosperity: "economic well-being",
  prosperous: "flourishing",
  protect: "defend",
  protection: "refuge",
  protector: "guardian",
  protocol: "convention",
  protraction: "lengthening",
  protrude: "stick out",
  protuberance: "bulge",
  protuberate: "bulge out",
  prove: "demonstrate",
  provide: "supply",
  province: "district",
  provision: "supply",
  provisional: "temporary",
  provocative: "stimulating",
  provoke: "stimulate",
  proximity: "nearness",
  prude: "overly proper person",
  prudence: "discretion",
  prudent: "discreetly wise",
  prudish: "priggish",
  pseudonym: "fictitious name",
  psychedelic: "hallucinatory",
  psychological: "mental",
  psychosis: "mental disorder",
  psychotic: "deranged",
  pub: "saloon",
  pucker: "wrinkle",
  pugnacious: "quarrelsome",
  pulsate: "throb",
  pulsation: "throb",
  pulverize: "crush",
  pummel: "punch",
  punctual: "prompt",
  puncture: "pierce",
  puny: "small",
  pup: "young dog",
  pupil: "student",
  purge: "cleanse",
  purify: "cleanse",
  purpose: "aim",
  purposeful: "intentional",
  purposely: "intentionally",
  pursue: "follow",
  pursuit: "chase",
  purview: "scope",
  push: "shove",
  pushy: "aggressive",
  pustule: "pimple",
  putrefaction: "rotting",
  putrefy: "rot",
  putrescence: "rottenness",
  putrid: "rotten",
  puzzlement: "perplexity",
  pyrotechnics: "fireworks",
  quadrennium: "four years",
  quadruped: "four-footed",
  quadruplicate: "fourfold",
  quagmire: "bog",
  quaint: "old-fashioned",
  quandary: "dilemma",
  quantity: "amount",
  quash: "suppress",
  quaver: "tremble",
  queasy: "nauseous",
  quench: "extinguish",
  query: "question",
  quest: "pursuit",
  questionable: "doubtful",
  questionnaire: "survey",
  quickly: "rapidly",
  quicksilver: "mercury",
  quietude: "tranquility",
  quill: "feather",
  quilt: "bed comforter",
  quintuple: "fivefold",
  quip: "clever remark",
  quiver: "tremble",
  quiz: "test",
  quote: "cite",
  rabid: "frenzied",
  racketeer: "mobster",
  rackety: "noisy",
  raconteur: "storyteller",
  racy: "indecently suggestive",
  radiance: "brightness",
  radiant: "beaming",
  radiate: "shine",
  radical: "extremist",
  rafter: "roof beam",
  rage: "fury",
  raging: "furious",
  ragtag: "scruffy",
  raisin: "dried grape",
  ramble: "wander",
  rambunctious: "boisterous",
  ramification: "consequence",
  ramshackle: "dilapidated",
  ranch: "cattle farm",
  rancid: "rank",
  rapid: "fast",
  rapidity: "swiftness",
  rapidly: "quickly",
  rapscallion: "rascal",
  raptor: "bird of prey",
  rarely: "seldom",
  rascal: "scoundrel",
  rating: "evaluation",
  ratio: "proportion",
  ration: "allotment",
  rationale: "explanation",
  rationality: "reasonableness",
  rationalize: "justify",
  ratty: "unkempt",
  raucous: "rowdy",
  ravage: "devastate",
  rave: "speak wildly",
  ravenous: "starved",
  ravine: "gorge",
  raving: "delirious",
  ravishing: "beautiful",
  reaction: "response",
  really: "truly",
  realm: "domain",
  rebellion: "uprising",
  rebellious: "insubordinate",
  rebuke: "chide",
  rebut: "contradict",
  rebuttal: "counter-argument",
  recede: "withdraw",
  receptacle: "container",
  receptive: "open",
  reciprocal: "mutual",
  reciprocate: "repay",
  reckless: "foolhardy",
  reckon: "calculate",
  recline: "lean back",
  recruit: "enlist",
  rectify: "correct",
  recuperate: "convalesce",
  recuperation: "convalescence",
  redundant: "superfluous",
  reek: "stink",
  referee: "umpire",
  referendum: "vote",
  refined: "polished",
  refrigerate: "chill",
  refrigerator: "icebox",
  refuge: "shelter",
  refurbish: "renovate",
  refute: "rebut",
  regal: "kingly",
  regatta: "boat race",
  regime: "government",
  registrar: "record-keeper",
  regress: "revert",
  regretful: "sorry",
  regrettable: "unfortunate",
  regrettably: "unfortunately",
  regurgitate: "vomit",
  rehearsal: "practice",
  reign: "rule",
  reimburse: "pay back",
  reincarnation: "rebirth",
  reinforce: "strengthen",
  reinforcement: "support",
  reiterate: "repeat",
  rejoice: "be delighted",
  relevant: "pertinent",
  relic: "ancient memento",
  relish: "enjoy",
  relocate: "move",
  reluctance: "unwillingness",
  reluctant: "unwilling",
  rely: "depend",
  remain: "stay",
  remark: "comment",
  remarkable: "extraordinary",
  remarkably: "unusually",
  remedial: "corrective",
  remedy: "cure",
  remember: "recollect",
  reminisce: "remember",
  remission: "temporary lessening",
  remittance: "payment",
  remnant: "leftover",
  remorse: "self-reproach",
  remorseful: "sorry",
  remote: "distant",
  rendezvous: "meeting place",
  renegade: "rebel",
  renounce: "give up",
  renovate: "restore",
  renown: "fame",
  renowned: "famous",
  reparation: "amends",
  replenish: "refill",
  replica: "copy",
  replicate: "copy",
  replication: "copy",
  reply: "respond",
  repository: "storehouse",
  reprehensible: "blameworthy",
  repress: "subdue",
  repressive: "inhibitory",
  reprisal: "retaliation",
  reproduce: "duplicate",
  reproduction: "copy",
  repudiate: "reject",
  repugnance: "loathing",
  repulse: "drive back",
  reputed: "supposed",
  requisition: "demand",
  resemblance: "similarity",
  resent: "begrudge",
  resentment: "bitterness",
  reside: "dwell",
  residence: "home",
  resign: "give up",
  resist: "oppose",
  resistance: "opposition",
  resolute: "firm",
  resound: "echo",
  respiration: "breathing",
  respire: "breathe",
  respond: "reply",
  response: "reply",
  responsibility: "duty",
  responsible: "accountable",
  restitution: "amends",
  restless: "uneasy",
  restrict: "limit",
  restriction: "limitation",
  result: "outcome",
  resurrect: "revive",
  resuscitate: "revive",
  retain: "keep",
  retract: "withdraw",
  retribution: "vengeance",
  retrieval: "recovery",
  revamp: "renovate",
  reveal: "disclose",
  revelry: "merrymaking",
  revenue: "income",
  reverberate: "echo",
  revere: "worship",
  reverie: "daydream",
  reviewer: "commentator",
  revoke: "withdraw",
  revolt: "rebel",
  revolting: "loathsome",
  revolve: "go around",
  revolver: "pistol",
  revulsion: "disgust",
  reword: "paraphrase",
  rhetorician: "orator",
  rhinal: "nasal",
  rhythm: "musical beat",
  rhythmic: "cadenced",
  ribaldry: "vulgar humor",
  riches: "wealth",
  ricochet: "rebound",
  ridiculous: "absurd",
  riffraff: "rabble",
  rig: "equip",
  rigid: "stiff",
  rigor: "severity",
  rile: "upset",
  rind: "peel",
  ringlet: "curl",
  rinse: "wash",
  riotous: "turbulent",
  ripen: "mature",
  ritzy: "posh",
  rival: "competitor",
  rivalry: "competition",
  riveting: "fascinating",
  roam: "wander",
  roast: "bake",
  robe: "gown",
  robust: "vigorous",
  rod: "pole",
  rogue: "scoundrel",
  romance: "love affair",
  romp: "frolic",
  roomer: "lodger",
  roost: "perch",
  rot: "decay",
  rotten: "decayed",
  rotund: "round",
  rotunda: "round building",
  rotundity: "roundness",
  roughly: "approximately",
  rouse: "awaken",
  routine: "ordinary",
  rove: "wander",
  rover: "wanderer",
  rowdy: "raucous",
  rubbish: "trash",
  rubella: "German measles",
  rubeola: "measles",
  ruckus: "commotion",
  rudimentary: "fundamental",
  ruffian: "bully",
  rug: "carpet",
  ruinous: "destructive",
  rummage: "search",
  rumor: "hearsay",
  rumple: "wrinkle",
  rumpus: "commotion",
  rundown: "summary",
  runoff: "overflow",
  rupture: "break",
  rural: "countrified",
  ruse: "trick",
  rust: "corrode",
  rut: "groove",
  ruthless: "cruel",
  sabotage: "undermine",
  sac: "small pouch",
  saccharine: "sugary",
  sack: "bag",
  sacrilege: "blasphemy",
  sacrilegious: "profane",
  sadden: "depress",
  safari: "hunting expedition",
  safeguard: "protect",
  sag: "droop",
  saga: "heroic story",
  sage: "wise person",
  salary: "pay",
  saline: "salty",
  saliva: "spit",
  saloon: "barroom",
  salutation: "greeting",
  salutatory: "welcoming address",
  salve: "ointment",
  same: "identical",
  sanctimonious: "self-righteous",
  sanctity: "holiness",
  sanctuary: "refuge",
  sanitary: "hygienic",
  sanitize: "cleanse",
  sarcastic: "derisive",
  sarcophagus: "stone coffin",
  sardonic: "mocking",
  sass: "backtalk",
  sassy: "impudent",
  satanic: "demonic",
  satiety: "fullness",
  satirical: "mocking",
  satirize: "ridicule",
  satisfactory: "acceptable",
  saturate: "soak",
  saunter: "stroll",
  savagery: "barbarity",
  savanna: "grassland",
  savant: "knowledgeable scholar",
  savor: "relish",
  savory: "tasty",
  scabbard: "sheath",
  scalawag: "rascal",
  scamp: "rascal",
  scan: "skim",
  scandalous: "disgraceful",
  scant: "meager",
  scanty: "insufficient",
  scarce: "rare",
  scarcity: "lack",
  scaremonger: "alarmist",
  scathing: "severely critical",
  scatter: "disperse",
  scent: "odor",
  scepter: "royal staff",
  schematic: "diagram",
  scheme: "plot",
  scholarship: "learned knowledge",
  scission: "splitting",
  scoot: "dash",
  scorch: "singe",
  scorn: "contempt",
  scornful: "contemptuous",
  scoundrel: "villain",
  scour: "clean",
  scowl: "frown",
  scraggly: "unkempt",
  scrape: "grate",
  scratchy: "prickly",
  scrawny: "skinny",
  scribble: "scrawl",
  scribe: "writer",
  scrooge: "miser",
  scrounge: "forage",
  scrub: "scour",
  scruffy: "unkempt",
  scrumptious: "delicious",
  scrutinize: "examine",
  scrutiny: "examination",
  sculpture: "carving",
  scurry: "scamper",
  scuttlebutt: "gossip",
  seabed: "ocean floor",
  seamy: "sordid",
  seasoning: "spice",
  secede: "withdraw",
  secession: "withdrawal",
  seclude: "isolate",
  secluded: "isolated",
  secondhand: "used",
  sect: "faction",
  sector: "region",
  secular: "worldly",
  secure: "safe",
  sedate: "calm",
  sedative: "tranquilizer",
  sediment: "dregs",
  seduction: "enticement",
  seductive: "alluring",
  seek: "search for",
  seem: "appear",
  seemly: "becoming",
  seep: "ooze",
  seepage: "oozing",
  seer: "prophet",
  seething: "boiling",
  segment: "section",
  segregate: "set apart",
  seldom: "rarely",
  selfless: "altruistic",
  selfsame: "identical",
  semblance: "likeness",
  senior: "elder",
  sensation: "feeling",
  sensational: "spectacular",
  sensor: "detector",
  sentiment: "opinion",
  sentimentality: "over-emotionalism",
  sentinel: "guard",
  sentry: "guard",
  septic: "infected",
  sequel: "continuation",
  sequence: "series",
  sequential: "serial",
  sequester: "seclude",
  serene: "calm",
  serenity: "peacefulness",
  serigraph: "silk-screen print",
  serpent: "snake",
  serviceable: "useful",
  servile: "submissive",
  serving: "portion",
  servitor: "attendant",
  setback: "reverse",
  settler: "colonist",
  sever: "divide",
  severe: "harsh",
  sew: "stitch",
  shackle: "fetter",
  shallow: "not deep",
  sham: "counterfeit",
  shambles: "mess",
  shanty: "hut",
  sharpshooter: "marksman",
  shawl: "cloak",
  sheaf: "bundle",
  shear: "cut",
  sheath: "case",
  sheen: "brightness",
  sheepish: "embarrassed",
  shenanigan: "mischief",
  shifty: "evasive",
  shiitake: "mushroom",
  shimmer: "gleam",
  shirk: "avoid",
  shoddy: "inferior",
  shortfall: "deficit",
  shorthand: "stenography",
  shout: "yell",
  shove: "push",
  shovel: "dig",
  shrewd: "astute",
  shriek: "scream",
  shrill: "piercing",
  shrivel: "wither",
  shrub: "bush",
  shuck: "husk",
  shuffle: "jumble together",
  shun: "avoid",
  sidekick: "pal",
  sidesplitting: "hilarious",
  sidestep: "evade",
  sierra: "mountain range",
  siesta: "afternoon nap",
  sieve: "strainer",
  significance: "meaning",
  significant: "important",
  signify: "mean",
  silhouette: "outline",
  silt: "sediment",
  silverback: "male gorilla",
  similar: "like",
  similarity: "likeness",
  similitude: "likeness",
  simpleton: "fool",
  simultaneous: "concurrent",
  sinful: "wicked",
  singe: "burn slightly",
  singer: "vocalist",
  singly: "individually",
  sinister: "evil",
  site: "location",
  situate: "position",
  skeletal: "gaunt",
  skeptic: "doubter",
  sketchy: "superficial",
  skiff: "small boat",
  skillet: "frying pan",
  skillful: "adept",
  skimpy: "meager",
  skinflint: "miser",
  skipper: "boat captain",
  skirmish: "clash",
  skitter: "glide rapidly",
  skittish: "nervous",
  skulk: "lurk",
  skullduggery: "trickery",
  skyline: "horizon",
  slack: "loose",
  slacken: "slow up",
  slam: "bang",
  slander: "speak badly of",
  slang: "jargon",
  slapdash: "haphazard",
  slash: "cut",
  slather: "spread thickly",
  slaughter: "killing",
  slay: "kill",
  sleazy: "squalid",
  sleek: "glossy",
  sleepy: "drowsy",
  sleet: "freezing rain",
  slender: "thin",
  slick: "glossily smooth",
  slicker: "raincoat",
  slim: "thin",
  slime: "ooze",
  slipshod: "slovenly",
  slit: "incision",
  slither: "glide",
  sliver: "splinter",
  slobber: "drool",
  slog: "tramp",
  slogan: "motto",
  sloppy: "messy",
  sloth: "laziness",
  slothful: "lazy",
  slouch: "droop",
  slovenly: "unkempt",
  sludge: "muck",
  sluggard: "lazy person",
  slumber: "sleep",
  slumberous: "sleepy",
  sly: "wily",
  smidgen: "iota",
  smirk: "sneer",
  smite: "strike",
  smithereens: "fragments",
  smug: "self-satisfied",
  snag: "obstacle",
  snake: "serpent",
  snare: "trap",
  snatch: "grab",
  snide: "sneering",
  snip: "cut",
  snippet: "small piece",
  snitch: "informer",
  snivel: "whine",
  snobby: "elitist",
  snoop: "spy",
  snoopy: "prying",
  snug: "cozy",
  soak: "drench",
  soar: "fly high",
  soaring: "lofty",
  sod: "turf",
  software: "computer programs",
  soggy: "wet",
  solace: "consolation",
  solarium: "sunroom",
  sole: "only",
  solemn: "grave",
  solicit: "ask",
  solidarity: "unity",
  solitary: "lone",
  solitude: "seclusion",
  solo: "alone",
  soluble: "easily dissolved",
  somber: "gloomy",
  somnambulate: "sleepwalk",
  somnambulist: "sleepwalker",
  somnolent: "drowsy",
  sonata: "musical composition",
  sonnet: "poem",
  sonny: "lad",
  sonogram: "ultrasound image",
  sonority: "resonance",
  soothe: "comfort",
  soothsayer: "foreteller",
  soothsaying: "prophecy",
  sophisticated: "worldly-wise",
  sorcerer: "wizard",
  sorcery: "witchcraft",
  sorority: "women's association",
  sorrow: "grief",
  sorrowful: "sad",
  source: "origin",
  sourpuss: "grump",
  souvenir: "memento",
  sovereignty: "dominion",
  spacious: "roomy",
  spangle: "glitter",
  sparing: "frugal",
  sparse: "scanty",
  spasm: "muscular contraction",
  spasmodic: "convulsive",
  spat: "quarrel",
  spawn: "breed",
  specially: "particularly",
  specific: "particular",
  specify: "designate",
  speckle: "dot",
  spectacular: "sensational",
  specter: "apparition",
  speculation: "conjecture",
  speculative: "theoretical",
  speechify: "harangue",
  spellbind: "bewitch",
  spellbinding: "mesmerizing",
  spellbound: "fascinated",
  spermaceti: "sperm whale oil",
  spew: "disgorge",
  spherical: "ball-shaped",
  spiffy: "stylish",
  spigot: "faucet",
  spike: "impale",
  spine: "backbone",
  spiny: "thorny",
  spire: "steeple",
  spirited: "lively",
  spiritual: "religious",
  spit: "saliva",
  spite: "malice",
  spiteful: "malicious",
  splashy: "flamboyant",
  splay: "spread apart",
  splendid: "magnificent",
  splendiferous: "glorious",
  splendor: "brilliancy",
  splice: "interweave",
  splinter: "sliver",
  splotch: "spot",
  spoilsport: "killjoy",
  spongy: "absorbent",
  spoof: "parody",
  sporadic: "intermittent",
  sporty: "flashy",
  spotless: "immaculate",
  spree: "fling",
  sprightly: "lively",
  springy: "resilient",
  sprint: "dash",
  spritz: "quick spray",
  spry: "nimble",
  spud: "potato",
  spunky: "spirited",
  spurt: "gush",
  squabble: "quarrel",
  squander: "waste",
  squash: "crush",
  squelch: "quell",
  squirm: "wriggle",
  stabilize: "steady",
  stack: "pile",
  stadium: "arena",
  stagger: "totter",
  stagnant: "motionless",
  stalemate: "deadlock",
  stallion: "male horse",
  stalwart: "strong",
  stamina: "endurance",
  stance: "posture",
  stanch: "halt",
  standstill: "stop",
  starchy: "stiff",
  stare: "gaze",
  stargazer: "astronomer",
  stash: "cache",
  stasis: "equilibrium",
  stately: "grand",
  stationary: "fixed",
  stationery: "writing materials",
  statuesque: "grand",
  statuette: "figurine",
  statute: "law",
  stead: "place",
  steadfast: "firm",
  stealth: "furtiveness",
  stealthy: "furtive",
  steamy: "muggy",
  steed: "horse",
  steeple: "spire",
  stein: "beer mug",
  stenography: "shorthand",
  stern: "austere",
  sternum: "breastbone",
  stickup: "armed robbery",
  stiff: "rigid",
  stifle: "smother",
  stigma: "mark of disgrace",
  stiletto: "dagger",
  stingy: "ungenerous",
  stink: "smell bad",
  stipulate: "specify",
  stipulation: "condition",
  stitch: "sew",
  stockpile: "reserve",
  stocky: "stout",
  stoop: "bend forward",
  straddle: "bestride",
  straggle: "wander",
  stratagem: "ploy",
  stray: "wander",
  streamlined: "aerodynamic",
  stretch: "extend",
  strew: "scatter",
  stricken: "afflicted",
  strife: "discord",
  striking: "impressive",
  stroll: "leisurely walk",
  stronghold: "fortress",
  stubborn: "obstinate",
  student: "pupil",
  stumble: "trip",
  stun: "daze",
  stunning: "striking",
  stupendous: "astounding",
  stupor: "daze",
  stylish: "fashionable",
  stymie: "obstruct",
  suave: "smooth in manner",
  subaqueous: "underwater",
  subcutaneous: "under the skin",
  subdue: "conquer",
  subdued: "muted",
  subjection: "conquest",
  subjoin: "append",
  submersed: "underwater",
  submission: "meek compliance",
  submissive: "yielding",
  submit: "yield",
  subsequent: "later",
  subside: "diminish",
  subsidy: "aid",
  substandard: "inferior",
  substantiate: "verify",
  substitute: "exchange",
  subterranean: "underground",
  subtract: "deduct",
  succession: "sequence",
  successive: "consecutive",
  succulent: "juicy",
  suckle: "breastfeed",
  sucrose: "sugar",
  suds: "froth",
  sufferance: "toleration",
  sufficient: "enough",
  suffocate: "smother",
  suicidal: "self-destructive",
  suitable: "appropriate",
  suitor: "wooer",
  sulk: "pout",
  sullen: "morose",
  summit: "peak",
  summon: "call",
  sumptuous: "lavish",
  sunbaked: "parched",
  sunken: "submerged",
  superb: "excellent",
  superficial: "shallow",
  superfluity: "excess",
  superintend: "administer",
  superintendent: "overseer",
  superlative: "excellent",
  supersede: "replace",
  supervise: "oversee",
  supplement: "add-on",
  supplemental: "auxiliary",
  supplementary: "additional",
  supply: "furnish",
  supposal: "assumption",
  suppress: "inhibit",
  supreme: "greatest",
  surely: "certainly",
  surgery: "operation",
  surmount: "overcome",
  surpass: "exceed",
  surplus: "excess",
  surreal: "dreamlike",
  surrealistic: "dreamlike",
  surrender: "yield",
  surrogate: "substitute",
  surround: "encircle",
  surroundings: "environment",
  suspense: "uncertainty",
  suspicion: "doubt",
  sustain: "support",
  sustenance: "food",
  swab: "mop",
  swaddle: "wrap in cloth",
  swagger: "strut",
  swamp: "marsh",
  swampy: "marshy",
  swank: "stylishness",
  swap: "exchange",
  swashbuckler: "adventurer",
  swashbuckling: "dashingly adventurous",
  swear: "curse",
  sweaty: "perspiring",
  sweep: "brush",
  sweepstakes: "lottery",
  sweetheart: "darling",
  swelter: "broil",
  sweltering: "hot",
  swerve: "veer",
  swig: "gulp",
  swill: "slop",
  swimmingly: "smoothly",
  swindle: "cheat",
  swine: "hogs",
  symbolize: "represent",
  symmetrical: "proportionate",
  symmetry: "balance",
  sympathize: "commiserate",
  sympathy: "pity",
  symposium: "discussion",
  symptomatic: "indicative",
  synagogue: "temple",
  synchronism: "simultaneousness",
  synopsis: "summary",
  synthetic: "artificial",
  systematic: "methodical",
  tableland: "plateau",
  taboo: "forbidden",
  tactful: "diplomatic",
  tactic: "maneuver",
  tactless: "rude",
  tadpole: "polliwog",
  tale: "story",
  talon: "claw",
  tang: "zest",
  tangible: "touchable",
  tangle: "snarl",
  tantalize: "tease",
  tardy: "late",
  tarnish: "stain",
  tart: "sour",
  tasty: "appetizing",
  tattered: "ragged",
  tattle: "divulge",
  tattletale: "informer",
  taunt: "ridicule",
  taupe: "brownish",
  taut: "tight",
  tawdry: "gaudy",
  tawny: "yellowish-brown",
  taxonomy: "classification",
  teal: "blue-green",
  tear: "rip",
  tearful: "weeping",
  tedious: "boring",
  teenage: "adolescent",
  teensy: "minuscule",
  teeter: "wobble",
  teeterboard: "seesaw",
  temperament: "disposition",
  temperamental: "moody",
  temperance: "moderation",
  temperate: "moderate",
  tempest: "storm",
  tempo: "pace",
  temporary: "not permanent",
  tenacity: "persistence",
  tenant: "occupant",
  tendency: "inclination",
  tenderfoot: "novice",
  tendon: "sinew",
  tepid: "lukewarm",
  termination: "end",
  terrain: "ground",
  terrestrial: "earthly",
  terrify: "frighten",
  terror: "fear",
  testy: "irritable",
  thankless: "unappreciated",
  thanks: "gratitude",
  thaw: "melt",
  theology: "divinity",
  theorem: "proposition",
  theorize: "speculate",
  theory: "hypothesis",
  therapeutic: "curative",
  therapy: "treatment",
  thorax: "insect midsection",
  thoroughfare: "passage",
  thoughtless: "inconsiderate",
  threaten: "menace",
  threefold: "triple",
  thrift: "frugality",
  thriftless: "wasteful",
  thrifty: "frugal",
  thrill: "excitement",
  thrive: "flourish",
  thriving: "prosperous",
  throb: "pulsate",
  throttle: "choke",
  thug: "ruffian",
  thunderous: "deafening",
  tibia: "shinbone",
  tic: "twitching",
  tidbit: "morsel",
  tidings: "news",
  tidy: "neat",
  tier: "level",
  tightwad: "miser",
  till: "cultivate",
  tilt: "slant",
  timeless: "eternal",
  timepiece: "watch",
  timetable: "schedule",
  timeworn: "hackneyed",
  timid: "shy",
  tinker: "fiddle",
  tiresome: "tedious",
  titanic: "colossal",
  titter: "giggle",
  tizzy: "dither",
  toddler: "young child",
  toil: "labor",
  toilsome: "laborious",
  tolerable: "endurable",
  tolerate: "allow",
  tomb: "grave",
  tomfoolery: "folly",
  tone: "sound",
  topic: "subject",
  topple: "overturn",
  torrent: "flood",
  toss: "fling",
  tot: "child",
  totalitarianism: "absolutism",
  totality: "sum",
  totter: "stagger",
  touchy: "irritable",
  tourist: "sightseer",
  tousle: "dishevel",
  toxic: "poisonous",
  toxin: "poison",
  trachea: "windpipe",
  traction: "adhesion",
  tragic: "very sad",
  traipse: "walk about",
  traitor: "betrayer",
  tram: "streetcar",
  tramp: "vagrant",
  trample: "stomp",
  trance: "hypnotic state",
  tranquil: "calm",
  tranquility: "calmness",
  tranquilize: "sedate",
  transcribe: "copy",
  transfiguration: "metamorphosis",
  transfix: "spellbind",
  transgress: "violate",
  transgression: "violation",
  transient: "fleeting",
  transit: "passage",
  translate: "interpret",
  translucent: "clear",
  transmit: "convey",
  transmogrify: "metamorphose",
  transmutation: "conversion",
  transparent: "clear",
  transpire: "happen",
  transport: "carry",
  transpose: "interchange",
  trash: "rubbish",
  trashy: "inferior",
  traumatic: "distressing",
  traumatize: "shock",
  treacherous: "dangerously untrustworthy",
  treachery: "perfidy",
  tread: "step",
  treatment: "handling",
  treaty: "agreement",
  tremble: "quiver",
  trench: "ditch",
  trendy: "fashionable",
  trepidation: "fear",
  trespass: "intrude",
  tribunal: "court",
  trickery: "deception",
  trickle: "dribble",
  trifle: "thing of little value",
  trifling: "insignificant",
  trio: "three",
  triple: "threefold",
  triplet: "threesome",
  triumph: "victory",
  trivet: "three-legged stand",
  trivial: "insignificant",
  troglodyte: "cave-dweller",
  trolley: "streetcar",
  trophy: "prize",
  trot: "jog",
  troubadour: "minstrel",
  trousers: "pants",
  truce: "armistice",
  trudge: "plod",
  truism: "axiom",
  truly: "sincerely",
  truncate: "shorten",
  trusty: "reliable",
  try: "attempt",
  tryout: "audition",
  tube: "pipe",
  tuft: "clump",
  tumble: "topple",
  tumbler: "drinking glass",
  tumultuous: "turbulent",
  tune: "melody",
  tuneful: "melodious",
  turbulent: "tumultuous",
  turnabout: "reversal",
  turquoise: "blue-green",
  turret: "little tower",
  tush: "backside",
  tutor: "instructor",
  twaddle: "nonsense",
  twilight: "dusk",
  twine: "string",
  twirl: "spin",
  twit: "taunt",
  twitch: "jerk",
  twitter: "chirp",
  tycoon: "magnate",
  typeface: "font",
  typhoon: "hurricane",
  typical: "representative",
  typify: "epitomize",
  tyrannical: "despotic",
  tyranny: "dictatorship",
  ultimatum: "demand",
  ultra: "extreme",
  ultramarine: "deep blue",
  umpire: "referee",
  umpteen: "very many",
  unabridged: "complete",
  unadulterated: "pure",
  unambiguous: "clear",
  unanimous: "with all agreed",
  unavoidable: "inevitable",
  uncertainty: "doubt",
  unconditional: "absolute",
  unconscious: "insensible",
  uncouple: "disconnect",
  uncouth: "awkward",
  uncover: "reveal",
  undaunted: "courageous",
  under: "beneath",
  underfoot: "in the way",
  undergo: "experience",
  underhanded: "devious",
  underlying: "fundamental",
  undermine: "weaken",
  underpin: "support",
  underside: "bottom",
  understand: "comprehend",
  undertaker: "mortician",
  undertaking: "project",
  underwriter: "guarantor",
  undoing: "ruin",
  uneasy: "restless",
  unfamiliar: "strange",
  unflappable: "imperturbable",
  unfounded: "baseless",
  unification: "merger",
  unilateral: "one-sided",
  unimpeachable: "blameless",
  unique: "the only one",
  unite: "join",
  unity: "oneness",
  unlade: "offload",
  unleash: "release",
  unmask: "expose",
  unnerve: "faze",
  unpalatable: "distasteful",
  unruly: "disobedient",
  unsavory: "distasteful",
  unvarnished: "plain",
  unveil: "reveal",
  unwind: "relax",
  unwise: "foolish",
  upbringing: "rearing",
  upheaval: "disturbance",
  uphold: "support",
  upkeep: "maintenance",
  uprising: "rebellion",
  uptight: "tense",
  usual: "ordinary",
  usurer: "money-lender",
  utensil: "tool",
  utilitarian: "useful",
  utility: "usefulness",
  utmost: "greatest",
  vacant: "empty",
  vacate: "leave",
  vaccinate: "inoculate",
  vaccination: "inoculation",
  vacuum: "void",
  vagrant: "drifter",
  vague: "indefinite",
  vain: "conceited",
  vainglorious: "boastful",
  valiant: "courageous",
  valor: "bravery",
  valorous: "brave",
  valuate: "appraise",
  valuation: "appraisal",
  vanity: "conceit",
  vanquish: "conquer",
  vapor: "fog",
  vaporous: "ethereal",
  variable: "changeable",
  variance: "difference",
  variety: "assortment",
  varnish: "gloss",
  vast: "huge",
  vat: "tub",
  vaulted: "arched",
  vegetation: "plants",
  veil: "conceal",
  velocity: "speed",
  vend: "sell",
  vendetta: "feud",
  vendor: "seller",
  veneer: "gloss",
  vengeance: "retribution",
  venom: "poison",
  venomous: "poisonous",
  ventilate: "air out",
  venue: "location",
  veranda: "porch",
  verbalize: "express",
  verbatim: "word for word",
  verbiage: "wordiness",
  verbose: "wordy",
  verdict: "judgment",
  verge: "brink",
  verification: "proof",
  verify: "confirm",
  verity: "truth",
  vermilion: "bright red",
  versatile: "having many uses",
  vertex: "top",
  vertical: "upright",
  vertigo: "dizziness",
  veteran: "old-timer",
  veterinarian: "animal doctor",
  veto: "reject",
  vex: "irritate",
  vexation: "annoyance",
  viable: "workable",
  vibrant: "lively",
  vicinity: "proximity",
  vicious: "brutal",
  victimize: "prey on",
  victor: "winner",
  victorious: "triumphant",
  victory: "triumph",
  vigil: "watch",
  vigilant: "watchful",
  vigor: "energy",
  vile: "despicable",
  villainous: "wicked",
  villainy: "wickedness",
  vincible: "conquerable",
  vindicate: "justify",
  vindictive: "spiteful",
  vintner: "wine dealer",
  violin: "fiddle",
  viper: "venomous snake",
  virility: "manliness",
  virtually: "nearly",
  virtuosity: "excellence",
  vise: "clamping tool",
  visor: "eyeshade",
  vista: "view",
  vital: "essential",
  vitality: "energy",
  vivacious: "lively",
  vivacity: "liveliness",
  vivid: "intense",
  vocation: "occupation",
  vogue: "style",
  volatile: "changeable",
  volition: "free will",
  volley: "salvo",
  voltage: "electric potential",
  voluminous: "large",
  voluptuous: "sensual",
  voracious: "ravenous",
  vouch: "attest",
  vulgar: "coarse",
  vulgarism: "obscenity",
  vulgarity: "crudeness",
  wacky: "zany",
  waft: "drift",
  wail: "lament",
  waistcoat: "vest",
  waive: "relinquish",
  wallet: "billfold",
  wallop: "whack",
  wallow: "roll about",
  wanderlust: "urge to travel",
  wangle: "manipulate",
  wannabe: "aspirant",
  wares: "merchandise",
  warily: "cautiously",
  warmonger: "militarist",
  warrant: "guarantee",
  wary: "cautious",
  watchman: "guard",
  wayward: "perverse",
  wearisome: "tedious",
  weary: "tired",
  wed: "marry",
  wedlock: "marriage",
  wee: "tiny",
  weeny: "tiny",
  weep: "sob",
  weird: "odd",
  weirdo: "eccentric",
  whalebone: "baleen",
  wharf: "pier",
  whim: "impulse",
  whimsical: "amusingly playful",
  whine: "complain",
  whinny: "neigh",
  whittle: "pare",
  whodunit: "detective story",
  wholehearted: "enthusiastic",
  wholesome: "healthy",
  whoop: "shout",
  whopping: "huge",
  wigwam: "teepee",
  wile: "trick",
  willful: "headstrong",
  willowy: "graceful",
  wilt: "droop",
  wily: "crafty",
  wince: "flinch",
  windpipe: "trachea",
  witchcraft: "sorcery",
  withdraw: "take back",
  wither: "shrivel",
  withstand: "resist",
  witless: "foolish",
  wittingly: "knowingly",
  wobbly: "shaky",
  woeful: "miserable",
  wonder: "marvel",
  wonderful: "marvelous",
  wondrous: "fantastic",
  woo: "court",
  woodman: "forester",
  woozy: "lightheaded",
  workaday: "ordinary",
  worker: "laborer",
  worsen: "deteriorate",
  wrangle: "brawl",
  wrapper: "covering",
  wrath: "anger",
  wrathful: "angry",
  wreath: "garland",
  wreck: "ruin",
  wretchedness: "misery",
  wring: "squeeze",
  wrinkle: "crease",
  writhe: "twist",
  xenophobia: "fear of foreigners",
  yearn: "hanker",
  yearning: "longing",
  yelp: "yip",
  yokel: "bumpkin",
  youngster: "child",
  yowl: "wail",
  yucky: "disgusting",
  yummy: "delicious",
  zany: "clownish",
  zap: "destroy",
  zeal: "fervor",
  zealotry: "fanaticism",
  zest: "gusto",
};

/* Capitals lookup */
var CapitalAnswers = {
  seoul: "South Korea",
  tehran: "Iran",
  oslo: "Norway",
  bangkok: "Thailand",
  moscow: "Russia",
  beirut: "Lebanon",
  amsterdam: "Netherlands",
  "san salvador": "El Salvador",
  khartoum: "Sudan",
  vienna: "Austria",
  helsinki: "Finland",
  reykjavik: "Iceland",
  stockholm: "Sweden",
  lima: "Peru",
  kingston: "Jamaica",
  prague: "Czech Republic",
  "new delhi": "India",
  baghdad: "Iraq",
  athens: "Greece",
  amman: "Jordan",
  brasilia: "Brazil",
  warsaw: "Poland",
  copenhagen: "Denmark",
  "washington d.c.": "United States",
  tokyo: "Japan",
  dublin: "Ireland",
  bogota: "Colombia",
  havana: "Cuba",
  madrid: "Spain",
  budapest: "Hungary",
  bern: "Switzerland",
  "kuwait city": "Kuwait",
  "panama city": "Panama",
  nairobi: "Kenya",
  nicosia: "Cyprus",
  ottawa: "Canada",
  rome: "Italy",
  pyongyang: "North Korea",
  tbilisi: "Georgia",
  "andorra la vella": "Andorra",
  lisbon: "Portugal",
  bissau: "Guinea-Bissau",
  santiago: "Chile",
  tripoli: "Libya",
  "mexico city": "Mexico",
  "buenos aires": "Argentina",
  cairo: "Egypt",
  mbabana: "Swaziland",
  nassau: "The Bahamas",
  kathmandu: "Nepal",
  manila: "Philippines",
  "kuala lumpur": "Malaysia",
  hanoi: "Vietnam",
  wellington: "New Zealand",
  canberra: "Australia",
  "guatemala city": "Guatemala",
  palikir: "Micronesia",
  "port-of-spain": "Trinidad and Tobago",
  ouagadougou: "Burkina Faso",
  vaduz: "Liechtenstein",
  "san jose": "Costa Rica",
  asuncion: "Paraguay",
  "saint john's": "Antigua and Barbuda",
  belgrade: "Serbia",
  valletta: "Malta",
  yerevan: "Armenia",
  bucharest: "Romania",
  kampala: "Uganda",
  "santo domingo": "Dominican Republic",
  dakar: "Senegal",
  "phnom penh": "Cambodia",
  "bandar seri begawan": "Brunei",
  algiers: "Algeria",
  vilnius: "Lithuania",
  kiev: "Ukraine",
  praia: "Cape Verde",
  manama: "Bahrain",
  kabul: "Afghanistan",
  bujumbura: "Burundi",
  male: "Maldives",
  mogadishu: "Somalia",
  chisinau: "Moldova",
  damascus: "Syria",
  zagreb: "Croatia",
  jakarta: "Indonesia",
  tallinn: "Estonia",
  "abu dhabi": "United Arab Emirates",
  doha: "Qatar",
  asmara: "Eritrea",
  montevideo: "Uruguay",
  berlin: "Germany",
  thimphu: "Bhutan",
  tunis: "Tunisia",
  tirane: "Albania",
  bratislava: "Slovakia",
  london: "England and the United Kingdom",
  "porto-novo": "Benin",
  "n'djamena": "Chad",
  dhaka: "Bangladesh",
  vientiane: "Laos",
  ulaanbaatar: "Mongolia",
  maputo: "Mozambique",
  colombo: "Sri Lanka",
  "port-au-prince": "Haiti",
  quito: "Ecuador",
  "addis ababa": "Ethiopia",
  monrovia: "Liberia",
  baku: "Azerbaijan",
  sarajevo: "Bosnia and Herzegovina",
  brussels: "Belgium",
  belmopan: "Belize",
  minsk: "Belarus",
  caracas: "Venezuela",
  ankara: "Turkey",
  riga: "Latvia",
  luanda: "Angola",
  riyadh: "Saudi Arabia",
  islamabad: "Pakistan",
  "la paz": "Bolivia",
  georgetown: "Guyana",
  gaborone: "Botswana",
  pretoria: "South Africa",
  victoria: "Seychelles",
  sofia: "Bulgaria",
  accra: "Ghana",
  tarawa: "Kiribati",
  kinshasa: "Congo-Democratic Republic",
  bamako: "Mali",
  dili: "Timor-Leste",
  skopje: "Macedonia",
  rabat: "Morocco",
  malabo: "Equatorial Guinea",
  "saint george's": "Grenada",
  roseau: "Dominica",
  ashgabat: "Turkmenistan",
  lusaka: "Zambia",
  yaounde: "Cameroon",
  tashkent: "Uzbekistan",
  podgorica: "Montenegro",
  freetown: "Sierra Leone",
  tegucigalpa: "Honduras",
  lilongwe: "Malawi",
  basseterre: "Saint Kitts and Nevis",
  maseru: "Lesotho",
  kingstown: "Saint Vincent-Grenadines",
  niamey: "Niger",
  suva: "Fiji",
  windhoek: "Namibia",
  astana: "Kazakhstan",
  honiara: "Solomon Islands",
  apia: "Samoa",
  melekeok: "Palau",
  "port louis": "Mauritius",
  lome: "Togo",
  abuja: "Nigeria",
  dushanbe: "Tajikistan",
  funafuti: "Tuvalu",
  ljubljana: "Slovenia",
  castries: "Saint Lucia",
  paramaribo: "Suriname",
  yamoussoukro: "Cote d'Ivoire",
  kigali: "Rwanda",
  conakry: "Guinea",
  sanaa: "Yemen",
  bangui: "Central African Republic",
  banjul: "The Gambia",
  nouakchott: "Mauritania",
  muscat: "Oman",
  "nuku'alofa": "Tonga",
  bishkek: "Kyrgyzstan",
  managua: "Nicaragua",
  brazzaville: "Congo-Republic",
  bridgetown: "Barbados",
  harare: "Zimbabwe",
  libreville: "Gabon",
  antananarivo: "Madagascar",
  "port-vila": "Vanuatu",
  majuro: "Marshall Islands",
};

/* GO! */
var frBotInstance = new FRBot();
frBotInstance.run();
